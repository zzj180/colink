...
public class SharedPreferencesHelper {
    ...
    private static final float WAKEUP_THRESHOLD = -3.0999999046325684f;
    ...
    public boolean saveFloatValue(String key, float floatValue) {
        if (!TextUtils.isEmpty(key)) {
            // Logger.d(TAG, "!--->saveIntValue--"+intValue);
            mEditor.putFloat(key, floatValue);
        }
        return mEditor.commit();
    }

    public float getFloatValue(String key) {
        return sp.getFloat(key, WAKEUP_THRESHOLD);
    }
