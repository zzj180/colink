...
public class XmFmGuiService extends Service {
            ...
            switch (msg.what) {
                ...
                case MSG_START_XMFM_PLAYER:
                    Logger.d(TAG, "MSG_SET_PLAY_MODE--keyword = " + mKeyword + "; categoryId = "
                            + mCategoryId);
                    startXmFmPlayer(mKeyword, mCategoryId);
                    break;
                default:
                    break;
            }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            ...
            if (XmFmConfig.ACTION_START_XM_FM.equals(action)) {
                String artist = intent.getStringExtra(XmFmConfig.KEY_EXTRA_FM_ARTIST);
                String categoryName = intent.getStringExtra(XmFmConfig.KEY_EXTRA_FM_CATEGORY);
                mKeyword = intent.getStringExtra(XmFmConfig.KEY_EXTRA_FM_KEYWORD);
                Logger.d(TAG, "!--->onStartCommand:--keyword=" + mKeyword + "; category = "
                        + categoryName + ";  artist = " + artist);
                mCategoryId = XmFmCategoryDataUtil.getDefaultCategoryMap().get(categoryName);
    ...
    private boolean checkConnection() {
        boolean isXmFmInstalled =
                DeviceTool.checkApkExist(mContext, XmFmConfig.PACKAGE_NAME_XMLY_FM);
        Logger.d(TAG, "checkConnection----isXmFmInstalled = " + isXmFmInstalled);
        if (!isXmFmInstalled) {
            mRebindCount = MAX_REBIND_COUNT;
            Logger.w(TAG, "checkConnection--app:" + XmFmConfig.PACKAGE_NAME_XMLY_FM
  ...
  /*  private void startXmFmPlayerContinuePlay(boolean isPlayNext) {
    ...
    private void showXmFmPlayer() {
        ...
        ComponentName cn =
                new ComponentName(XmFmConfig.PACKAGE_NAME_XMLY_FM,
                		XmFmConfig.ACTIVITY_NAME_XMLY_FM_MAIN);
  ...
  /*  private void setPlayByAlbumTracks(CommonTrackList commonTrackList, int position) {
            ...
            try {
                ...
                if (CommandPreference.CMDPREVIOUS.equals(cmd)) {
                    ...
                    mUIHandler.sendEmptyMessageDelayed(MSG_PAUSE,
                    		XmFmConfig.TIME_SET_STATE_DELAY);
    ...
    private void registReceiver() {
        ...
        commandFilter.addAction(XmFmConfig.SERVICECMD);
