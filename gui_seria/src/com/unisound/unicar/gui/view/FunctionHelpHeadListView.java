 ...
 * @date 20150716
 */
public class FunctionHelpHeadListView extends LinearLayout implements ISessionView {

    private static final String TAG = FunctionHelpHeadListView.class.getSimpleName();
    ...
    public void findViews() {
        ...
        mFunctionHelpAdapter =
                new FunctionHelpViewAdapter(mContext, getHelpTextInfoList(isShowRandomHelpText));
    ...
    public void initFunctionViews() {
        ...
        // XD 20150807 modify
        mSupprotDomainList = FunctionHelpUtil.getUnsupportedHelpTextList(mContext);
    }
     ...
     * create Show Text
     * 
     * @param isShowRandomHelpText default： false; show resourceId arr[0] help text if true , must
     *        satisfy the needs of: resourceId arr[].size() > 1
     */
    private ArrayList<FunctionHelpTextInfo> getHelpTextInfoList(boolean isShowRandomHelpText) {
        ArrayList<FunctionHelpTextInfo> list = new ArrayList<FunctionHelpTextInfo>();

        Logger.d(TAG, "!--->getHelpTextInfoList()---isShowRandomHelpText--" + isShowRandomHelpText
                + "supprotList.size()  :" + mSupprotDomainList.size());
        if (mSupprotDomainList.size() > 0) {
            ...
            int[] numberArray = ArithmeticUtil.getRandomArray(0, size - 1, showNo);
            ...
            for (int i = 0; i < numberArray.length; i++) {
                ...
                if (isShowRandomHelpText) {
                    ...
                    s = KnowledgeMode.getRandomContentString(getContext(), sd.resourceId);
                    Logger.d(TAG, "!--->getHelpTextInfoList()--i :" + i + " numberArray[i]:"
                            + numberArray[i] + " s" + s);
                } else {
                ...
                // showTexts.add(s);
                s = FunctionHelpUtil.removeDoubleQuotationMarks(s);// xd added 20150729
                Logger.d(TAG, "!--->getHelpTextInfoList()--type :" + sd.type + "; s =" + s);
