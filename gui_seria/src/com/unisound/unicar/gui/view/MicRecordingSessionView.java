 ...
 * @date 20150709
 */
public class MicRecordingSessionView extends LinearLayout implements ISessionView {

    private static final String TAG = MicRecordingSessionView.class.getSimpleName();
    ...
    private void initView() {
        View view =
                LayoutInflater.from(mContext).inflate(R.layout.session_mic_status_recording_view,
                        this, true);
        ...
        mTvHelpLink.getPaint().setAntiAlias(true);
        mTvHelpLink.setOnClickListener(mLinkClickListener);

        mIvTestVersionMark = (ImageView) findViewById(R.id.iv_test_version_mark);
        mIvTestVersionMark.setVisibility(GUIConfig.isTestVersion ? View.VISIBLE : View.GONE);
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.tv_recording_help_link:
                    Logger.d(TAG, "!--->tv_recording_help_link click. start AllFunctionsActivity");
    ...
    /**
     * get Random show text list which size is COUNT_DEFAULT_SHOW from SupportDomain list
     * 
     * @return
     */
    private ArrayList<String> getShowTexts() {
        ...
        ArrayList<SupportDomain> supprotList =
                FunctionHelpUtil.getUnsupportedHelpTextList(mContext);
        /* XD 20150807 modify End > */
        int[] indexArray =
                ArithmeticUtil.getRandomArray(0, supprotList.size() - 1, COUNT_DEFAULT_SHOW);
        ...
        for (int i = 0; i < indexArray.length; i++) {
            ...
            if (SessionPreference.DOMAIN_CALL.equals(sd.type)
                ...
                // text = ContactsUtil.getHelpTextWithContactName(mContext, text);
            ...
            Logger.d(TAG, "!--->getShowTexts()--type :" + sd.type + "; text =" + text);
            showTexts.add(text);
        }
        return showTexts;
    }
