...
public class RomCustomerProcessing {
    ...
    public static void exitAppForOther(Context context, String packageName, String className) {
        Logger.d(TAG, "action = " + APP_ACTION + ";uninstallApp packageName=" + packageName
                + ";className = " + className);
    ...
    public static void sendMessageToOtherSetting(Context context, String operands, String operator) {
        Logger.d(TAG, "action = " + SETTING_ACTION + ";operands = " + operands + ";operator = "
                + operator);
        Intent intent = new Intent();
    ...
    public static void playtts(Context context, String ttsContent, int type) {
        ((WindowService) context).playTTs(ttsContent, type);
    }
}
