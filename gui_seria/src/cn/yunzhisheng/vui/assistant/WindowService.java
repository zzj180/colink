...
public class WindowService extends Service {
	...
	public static final int MSG_ON_START_RECORDING_FAKE_ANIMATION = 2014; // XD
																			// added
																			// 20151015
	...
	private MicrophoneControlDoresoView mMicrophoneDoresoControl; // XD added
																	// 20150828
	...
	private OnTouchEventListener mOnTouchEventListener = new OnTouchEventListener() {
		...
		public boolean onTouchEvent(MotionEvent event) {
			...
			if ((event.getAction() == MotionEvent.ACTION_DOWN)
					&& ((x < 0) || (x >= mViewRoot.getWidth()) || (y < 0) || (y >= mViewRoot.getHeight()))) {
				mSessionManager.cancelTalk();
				return true;
	...
	private OnCompletionListener mOnCompletionListener = new OnCompletionListener() {
		...
		public void onCompletion(MediaPlayer arg0) {
			...
			if (ismute) {
				...
				getAudioManager().setStreamVolume(AudioManager.STREAM_ALARM, 0, 0);
			}
			if (mPlayer != null) {
				mPlayer.release();
			}
	...
	public void onCreate() {
		...
		UserPerferenceUtil.VALUE_VERSION_MODE_DEFAULT = com.unisound.unicar.gui.utils.SystemPropertiesProxy.getInt(this, "ro.coogo.voice.mode",UserPerferenceUtil.VALUE_VERSION_MODE_EXP);
		 ...
		 * (View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN);// XD added } else {
...
*/
	...
	private void syncConfigure() {
		if(MessageReceiver.readAccFile()){
			int storageVolume = Settings.System.getInt(mContext.getContentResolver(),KEY_VOLUME_VALUE, 7);
			getAudioManager().setStreamVolume(AudioManager.STREAM_MUSIC,storageVolume * 2 + 1, 0);
			getAudioManager().setStreamVolume(AudioManager.STREAM_NOTIFICATION, storageVolume, 0);
			getAudioManager().setStreamVolume(AudioManager.STREAM_ALARM, storageVolume, 0);
			ContentValues con = new ContentValues();
			con.put(WakeUpDB.SWITCH, 1);
			...
			getAudioManager().setStreamVolume(AudioManager.STREAM_NOTIFICATION, 0, 0);
			getAudioManager().setStreamVolume(AudioManager.STREAM_ALARM, 0, 0);
			sendAccCloseEvent();
			ContentValues con = new ContentValues();
			con.put(WakeUpDB.SWITCH, 0);
		...
		sendVersionLevel(); // XD added 20151123
	}
	...
	private IMessageRouterCallback mCallback = new IMessageRouterCallback.Stub() {
		...
		public void onCallBack(String callBackJson) throws RemoteException {
			...
			if ("TW".endsWith(language)){
				callBackJson=ZHConverter.convert(callBackJson, ZHConverter.TRADITIONAL);
			}
	...
	/*public void sendResponse() {
		...
		String callID = JsonTool.getJsonValue(jobj,
				SessionPreference.KEY_CALL_ID);
		String param = "{\"functionID\":\"6\"}";
		messageTransCenter.sendResponse(callID, "onTTSPlayEnd", "SUCCESS", "", param);
	}*/
	...
	ISystemCallTransitionListener mMessageTransListener = new ISystemCallTransitionListener() {
		...
		public void onOneShotRecognizerTimeout() {
			Logger.d("mMessageTransListener","!--->---onOneShotRecognizerTimeout---");
			Message msg = new Message();
		...
		public void onClickMainActionButton(int style) {// add tyz 20151020 do
														// mian action branch
			// TODO Auto-generated method stub
			Logger.d("mMessageTransListener", "!--->onClickMainActionButton()----style = " + style);
			Message msg = new Message();
	...
	private void updateMicControlView(int type) {
		...
		if (SessionPreference.VALUE_SET_STATE_TYPE_WAKEUP_SUCCESS == type) {
			...
			mSessionManager.setMicrophoneControl(mMicrophoneControl); // XD
																		// 20150827
																		// added
			...
			mSessionManager.setMicrophoneControl(mMicrophoneDoresoControl); // XD
																			// 20150827
																			// added
		}
	}
		...
		public void handleMessage(android.os.Message msg) {
			...
			case MSG_SET_STATE:
				...
				case SessionPreference.VALUE_SET_STATE_TYPE_WRITE_CONTACT_INFO:
					Logger.d(TAG,
							"!--->VALUE_SET_STATE_TYPE_WRITE_CONTACT_INFO----");
					writeContactsInfo();
					break;
				...
				case SessionPreference.VALUE_SET_STATE_TYPE_GET_SUPPORT_DOMAIN_LIST:
					Logger.d(TAG,
							"!--->VALUE_SET_STATE_TYPE_GET_SUPPORT_DOMAIN_LIST----");
					// TODO:
	...
	private BroadcastReceiver mSmsSendStatusReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (GUIConfig.ACTION_SMS_SEND_SUCCESS.equals(intent.getAction())) {
				mSessionManager
						.showSmsSendStatusView(GUIConfig.SMS_STATUS_SEND_SUCCESS);
				...
				public void onLocationChanged(LocationInfo locationInfo,
					...
					if (locationInfo != null) {
						...
						if (mLocationInfo != null) {
							...
							sendProtocolEvent(SessionPreference.EVENT_NAME_GENERAL_GPS,"{\"gpsInfo\":" + "\""+ mLocationInfo.getLatitude() + ","
											+ mLocationInfo.getLongitude() + "\",\"city\":"+ mLocationInfo.getCity() + "}");
						}
					}
				}
			});
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (Intent.ACTION_SCREEN_ON.equals(action)) {
				...
				if (!MessageReceiver.readAccFile()) {
					...
					accSwitch = false;
					curValue = getAudioManager().getStreamVolume(AudioManager.STREAM_ALARM);
					updateVolumeValue(curValue);
					getAudioManager().setStreamVolume(AudioManager.STREAM_MUSIC,0, 0);
					getAudioManager().setStreamVolume(AudioManager.STREAM_NOTIFICATION,0, 0);
					getAudioManager().setStreamVolume(AudioManager.STREAM_ALARM, 0, 0);
				...
				int storageVolume = Settings.System.getInt(
						mContext.getContentResolver(),KEY_VOLUME_VALUE, 7);
				getAudioManager().setStreamVolume(AudioManager.STREAM_MUSIC,storageVolume * 2 + 1, 0);
				getAudioManager().setStreamVolume(AudioManager.STREAM_NOTIFICATION, storageVolume, 0);
				getAudioManager().setStreamVolume(AudioManager.STREAM_ALARM, storageVolume, 0);
				...
				if (mPlayer != null && !isPlaying && accSwitch) {
					...
					if (curValue > 0) {
						ismute = true;
						mPlayer.reset();
						AssetFileDescriptor afd = context.getResources().openRawResourceFd(R.raw.mute);
						try {
						...
						ismute = false;
						mPlayer.reset();
						AssetFileDescriptor afd = context.getResources().openRawResourceFd(R.raw.cancel_mute);
						try {
				...
				if(accSwitch){
					...
					sendPTT();
					sendResponse("");
					sendProtocolEvent(SessionPreference.EVENT_NAME_RECORDING_CONTROL,GuiProtocolUtil.getRecordingControlParamProtocol(SessionPreference.PARAM_RECORDING_CONTROL_STOP, SessionPreference.DOMAIN_ROUTE));
					sendProtocolEvent(SessionPreference.EVENT_NAME_UPDATE_POI_KEYWORD,GuiProtocolUtil.getChangeLocationParamProtocol(keyword));
				}
	...
	/*private void setDefaultLocation() {
		JSONObject mLocationJsonObject = JsonTool
				.parseToJSONObject(PrivatePreference.mDeufaultLocation);
		...
		mLocationInfo.setLongitude(JsonTool.getJsonValue(mLocationJsonObject,
				"lng", 121.401098));
		mLocationInfo.setType(JsonTool.getJsonValue(mLocationJsonObject,
				"type", 5));
		mLocationInfo.setAddress(JsonTool.getJsonValue(mLocationJsonObject,
				"address", "上海市 徐汇区 钦州北路 靠近电信恒联"));
		mLocationInfo.setDistrict(JsonTool.getJsonValue(mLocationJsonObject,
				"destrict", "徐汇区"));
		mLocationInfo.setCity(JsonTool.getJsonValue(mLocationJsonObject,
				"city", "上海"));
	}
*/
	...
	public void CreateDialog(Context context) {
		...
		((TextView) v.findViewById(R.id.textView1)).setText(getString(R.string.prepnavi_endpoi, 
				mUserPreference.getString("address", getString(R.string.prepnavi_address))));
		builder.setView(v);
	...
	private void CountDown() {
		...
		mCountDownTimer = new CountDownTimer(10000, 1000) {
			...
			public void onTick(long millisUntilFinished) {
				...
				button1.setText(getString(R.string.cancel_prepnavi, millisUntilFinished / 1000));
			}
	...
	private void startNavi() {
		...
		case 1:
			...
			break;
		case 3:
			Gps gcj = PositionUtil.bd09_To_Gcj02(
					mUserPreference.getFloat("lat", 0f),
					mUserPreference.getFloat("lng", 0f));
			...
			break;
		case 4:
        	Gps gps = PositionUtil.bd09_To_Gps84(mUserPreference.getFloat("lat", 0f),
					mUserPreference.getFloat("lng", 0f));
	...
	private void recover() {
		...
		if (volume < 1) {
			...
			getAudioManager().setStreamVolume(AudioManager.STREAM_MUSIC,
					volume * 2 + 1, AudioManager.FLAG_ALLOW_RINGER_MODES);
		}
	}
