...
public class FmCategoryDao extends BaseDao {
    ...
    public void add(CategoryModel category) {
        ...
        if (db.isOpen()) {
            ...
            values.put(DBConstant.FmCategoryTB.TABLE_ITEM_CATEGORY_ID, category.getCategoryId());
            values.put(DBConstant.FmCategoryTB.TABLE_ITEM_CATEGORY_NAME, category.getCategoryName());
     ...
     * 
     * @param categoryList
     * @param isUpdate: true:delete all old data before add
     */
    public void add(List<CategoryModel> categoryList, boolean isUpdate) {
        ...
        if (isUpdate && db.isOpen()) {
            ...
            db.execSQL("delete * from " + DBConstant.FmCategoryTB.TABLE_NAME_CATEGORY);
        }
    ...
    public void delete(String categoryName) {
        SQLiteDatabase db = getOpenDb();
        if (db.isOpen()) {
            db.execSQL("delete from " + DBConstant.FmCategoryTB.TABLE_NAME_CATEGORY + " where "
    ...
    public void update(CategoryModel category) {
        ...
        if (db.isOpen()) {
            ContentValues values = new ContentValues();
            values.put(DBConstant.FmCategoryTB.TABLE_ITEM_CATEGORY_ID, category.getCategoryId());
            values.put(DBConstant.FmCategoryTB.TABLE_ITEM_CATEGORY_NAME, categoryName);
            db.update(DBConstant.FmCategoryTB.TABLE_NAME_CATEGORY, values,
                    DBConstant.FmCategoryTB.TABLE_ITEM_CATEGORY_NAME, new String[] {categoryName});
        }
        db.close();
    }
    ...
    public ArrayList<CategoryModel> getAllEntries() {
        ...
        if (db.isOpen()) {
            CategoryModel category = null;
            Cursor cursor =
                    db.query(DBConstant.FmCategoryTB.TABLE_NAME_CATEGORY, null, null, null, null,
                            null, DBConstant.FmCategoryTB.TABLE_ITEM_CATEGORY_ID);
    ...
    public int getCagegoryIdByName(String categoryName) {
        int categoryId = 0;
        Logger.d(TAG, "!--->getCagegoryIdByName---categoryName = " + categoryName);
        ...
        try {
            ...
            if (db.isOpen()) {
                Cursor cursor =
                        db.query(DBConstant.FmCategoryTB.TABLE_NAME_CATEGORY, null,
                                DBConstant.FmCategoryTB.TABLE_ITEM_CATEGORY_NAME + " =?",
                ...
                categoryId =
                        cursor.getInt(cursor
                                .getColumnIndex(DBConstant.FmCategoryTB.TABLE_ITEM_CATEGORY_ID));
                if (null != cursor) {
                    cursor.close();
                }
            }
        ...
        if (db.isOpen()) {
            Cursor cursor =
                    db.query(
                            ...
                            null,
                            DBConstant.FmCategoryTB.TABLE_ITEM_CATEGORY_NAME + " =" + cagtegoryName,
                            null, null, null, null);
            cursor.moveToNext();
