...
public class RomCustomMessageReceiver extends BroadcastReceiver {
    ...
    private void resetContact() {
        ...
        Uri uri = Uri.parse("content://cn.yunzhisheng.vui.provider.ContactProvider/contact");
    ...
    private void resetPhone() {
        ...
        ContentResolver resolver = mContext.getContentResolver();
        Uri uri = Uri.parse("content://cn.yunzhisheng.vui.provider.ContactProvider/phone");
        resolver.delete(uri, null, null);
    }
    ...
    private void deleteContact(String name, String number) {
        ...
        Cursor cursor =
                resolver.query(uri, contactProjection, "display_name=?", new String[] {name}, null);
        if (cursor != null) {
    ...
    private boolean deletePhone(long id, String number) {
        ContentResolver resolver = mContext.getContentResolver();
        Uri phoneUri = Uri.parse("content://cn.yunzhisheng.vui.provider.ContactProvider/phone");
        String[] contactProjection = {"contact_id", "number"};
        Cursor cursor =
                resolver.query(phoneUri, contactProjection, "contact_id=?", new String[] {id + ""},
                        null);
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                do {
                    ...
                    if (phoneNumber != null && phoneNumber.equals(number)) {
                        resolver.delete(phoneUri, "contact_id=?", new String[] {id + ""});
                        cursor.close();
                        cursor = null;
                        return true;
                    }
