...
public class SettingSession extends BaseSession {
    ...
    public void putProtocol(JSONObject jsonProtocol) {
        ...
        String operands = JsonTool.getJsonValue(mDataObject, "operands", "");
    	...
    	if (SessionPreference.VALUE_SETTING_OBJ_CALL.equals(operands)) {		//电话
			...
			if (SessionPreference.VALUE_SETTING_ACT_ANSWER.equals(operator)) {
				...
				mAnswer = mContext.getString(R.string.redial);
		...
		/* XD 20150717 delete End >*/
			...
			if (SessionPreference.VALUE_SETTING_ACT_CANCEL.equals(operator)) {
				...
				intent.putExtra(CommandPreference.CMD_KEY_NAME, CommandPreference.CMD_NAME_SHUFFLE_PLAYBACK);
			...
			if (SessionPreference.VALUE_SETTING_ACT_PLAY.equals(operator)) {
				...
				Intent intent = new Intent(CommandPreference.ACTION_SERVICE_CMD);
				intent.putExtra(CommandPreference.CMD_KEY_NAME, CommandPreference.CMD_NAME_ORDER_PLAYBACK);
				mMessageSender.sendMessage(intent, null);
			}
			...
			if (SessionPreference.VALUE_SETTING_ACT_PLAY.equals(operator)) {
				...
				Intent intent = new Intent(CommandPreference.ACTION_SERVICE_CMD);
				intent.putExtra(CommandPreference.CMD_KEY_NAME, CommandPreference.CMD_NAME_SINGLE_CYCLE);
				mMessageSender.sendMessage(intent, null);
			...
			if (SessionPreference.VALUE_SETTING_ACT_PLAY.equals(operator)) {
				...
				Intent intent = new Intent(CommandPreference.ACTION_SERVICE_CMD);
				intent.putExtra(CommandPreference.CMD_KEY_NAME, CommandPreference.CMD_NAME_FULL_CYCLE);
				mMessageSender.sendMessage(intent, null);
			}
			...
			Intent intent = new Intent(CommandPreference.ACTION_SERVICE_CMD);
			intent.putExtra(CommandPreference.CMD_KEY_NAME, CommandPreference.CMD_NAME_CLOSE_DESK_LYRIC);
			mMessageSender.sendMessage(intent, null);
			...
			Intent intent = new Intent(CommandPreference.ACTION_SERVICE_CMD);
			intent.putExtra(CommandPreference.CMD_KEY_NAME, CommandPreference.CMD_NAME_OPEN_DESK_LYRIC);
			mMessageSender.sendMessage(intent, null);
			...
			if (SessionPreference.VALUE_SETTING_ACT_GO_BACK_HOME.equals(operator)) {
				...
				mAnswer = mContext.getString(R.string.one_navi);
				...
				mAnswer = mContext.getString(R.string.open_play);
				int mapIndex = UserPerferenceUtil.getMapChoose(mContext);
				switch (mapIndex) {
				case 1:
				...
				mAnswer = mContext.getString(R.string.close_play);
				int mapIndex = UserPerferenceUtil.getMapChoose(mContext);
				switch (mapIndex) {
				case 1:
				...
				mAnswer = mContext.getString(R.string.zoomin_map);
				int mapIndex = UserPerferenceUtil.getMapChoose(mContext);
				switch (mapIndex) {
				case 1:
				...
				mAnswer = mContext.getString(R.string.zoomout_map);
				int mapIndex = UserPerferenceUtil.getMapChoose(mContext);
				switch (mapIndex) {
				case 1:
			...
			if (SessionPreference.VALUE_SETTING_ACT_CLOSE.equals(operator)) {
				mAnswer = mContext.getString(R.string.close_screen);
		...
		//		RomControl.enterControl(mContext, RomControl.ROM_OPEN_MODEL_MUTE);
			...
			mAnswer = mContext.getString(R.string.camera_complain);
			mContext.sendOrderedBroadcast(new Intent(ACTION_COOGO_FEEDBACK), null);
		}else if (SessionPreference.VALUE_SETTING_OBJ_QUESTION.equals(operands)) { 
			if (SessionPreference.VALUE_SETTING_ACT_PLAY.equals(operator)) {
		...
		// 如果协议中包含OBJ_XXX,请在这里添加
			...
			/*if (operator.equals(SessionPreference.VALUE_SETTING_ACT_OPEN_CHANNEL)) {
				...
				Intent intent = new Intent(CommandPreference.ACTION_SERVICE_CMD);
				intent.putExtra(CommandPreference.CMD_KEY_NAME, CommandPreference.CMD_NAME_STOP);
				mMessageSender.sendMessage(intent, null);
...
/*			else if (SessionPreference.VALUE_SETTING_ACT_CLOSE.equals(operator)) {
    ...
    public static void forceStopPackage(String pkgName, Context context) {
		...
		Method method=null;
		try {
			method = Class.forName(ACTIVITY_MANAGER).getMethod("forceStopPackage", String.class);
