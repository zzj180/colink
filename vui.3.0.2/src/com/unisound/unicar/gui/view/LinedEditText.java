...
public class LinedEditText extends EditText {
	...
	public LinedEditText(Context context, AttributeSet attrs) {
		...
		int color = a
				.getColor(R.styleable.LinedEditText_line_color, 0XFFFFFFFF);
		float stokeWidth = a.getDimension(
				R.styleable.LinedEditText_line_stoke_width, 2);
		float dashOnWidth = a.getDimension(
				R.styleable.LinedEditText_line_dash_on_width, 2);
		float dashOffWidth = a.getDimension(
				R.styleable.LinedEditText_line_dash_off_width, 2);
		mDrawLineMode = a.getInt(R.styleable.LinedEditText_lines_according, 1);
		a.recycle();
		mPaint.setStyle(Paint.Style.STROKE);
		mPaint.setStrokeWidth(stokeWidth);
		PathEffect effects = new DashPathEffect(new float[] { dashOnWidth,
				dashOffWidth }, 1);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mDrawLineMode == 2) {
			int lines = (getHeight() - getPaddingTop() - getPaddingBottom())
					/ lineHeight;
