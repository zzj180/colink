...
public class PickLocationView extends PickBaseView {
	...
	public void setPickLocationViewListener(
			PickLocationViewListener pickLocationViewListener) {
	...
	public void initView(ArrayList<LocationInfo> locationInfos,
			String locationToPoi) {
		Logger.d(TAG, "initView---locationToPoi = " + locationToPoi);
		mLocationInfos = locationInfos;
		View header = mLayoutInflater.inflate(
				R.layout.pickview_header_location, this, false);
		TextView tvHead = (TextView) header
				.findViewById(R.id.tv_header_location_keyword);
		tvHead.setText(locationToPoi);

		ImageView ivEditLocation = (ImageView) header
				.findViewById(R.id.iv_edit_location);
	...
	private void addedMoreButton() {
		View buttomLay = mLayoutInflater.inflate(R.layout.list_item_load_more,
				null, false);
		mLoadMoreLay = (LinearLayout) buttomLay
	...
	public View getLocalItemView(LocationInfo contactInfo) {
		View view = mLayoutInflater.inflate(R.layout.pickview_item_location,
				mContainer, false);
		...
		if (TextUtils.isEmpty(name)) {
			...
			tvAddress.setTextColor(getContext().getResources().getColor(
					R.color.black));
		} else {
			tvName.setText(name);
		}
