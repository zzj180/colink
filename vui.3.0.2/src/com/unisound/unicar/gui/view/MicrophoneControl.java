...
public class MicrophoneControl extends MicrophoneControlBaseView {
	...
	public void setMicrophoneViewClickListener(
			MicrophoneViewClickListener listener) {
	...
	public MicrophoneControl(final Context context, AttributeSet attrs,
			int defStyle) {
		...
		mTextViewAnswer.setOnClickListener(mOnClickListener); // XD added
																// 20151014
		mContext = context;
		...
		mRotateAnimationMicRecognize
				.setDuration(TIME_MINISECONDS_DURATION_RECOGNIZE);
		...
		boolean isWakeUp = UserPerferenceUtil.isWakeupEnable(context);
	...
	public void setAnswerText(String text) {
		...
		mTextViewAnswer.setTextColor(getResources().getColor(
				R.color.color_asr_result_text));
	}
	 ...
	 */
	@Override
	public void showWakeupStatusOnMicView(boolean isWakeupOpen,
			String wakeupWord) {
		Logger.d(TAG, "!--->showWakeupStatusOnMicView--isWakeupOpen="
				+ isWakeupOpen + "; wakeupWord = " + wakeupWord);
		...
		if (isWakeupOpen) {
			...
			mTvWakeupWord.setText(FunctionHelpUtil
					.addDoubleQuotationMarks(wakeupWord));
		} else {
	...
	private void hideWakeupStatusView() {
		if (null != mWakeupWordLayout
				&& mWakeupWordLayout.getVisibility() == View.VISIBLE) {
			mWakeupWordLayout.setVisibility(View.GONE);
		}
		if (null != mTextViewAnswer
				&& mTextViewAnswer.getVisibility() != View.VISIBLE) {
	...
	public void setVoiceLevel(final int level) {
		...
		if (time - mLastVolumeUpdateTime < GUIConfig.TIME_MIC_VOLUME_UPDATE) {
			Logger.d(TAG, "!--->setVoiceLevel---less than  "
					+ GUIConfig.TIME_MIC_VOLUME_UPDATE
	...
	private void setFlickerAnimation(ImageView imageView) {
		final Animation animation = new AlphaAnimation(1, 0); // Change alpha
																...
																// visible to
																// invisible
		animation.setDuration(TIME_MINISECONDS_MIC_FLICKER); // duration - 1/5 a
																// second
