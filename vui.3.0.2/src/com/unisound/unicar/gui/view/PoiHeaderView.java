...
public class PoiHeaderView extends FrameLayout {
			...
			case R.id.textViewPoiSort:
				...
				if (mPopupWindowSort == null) {
					...
					for (int i = 0; i < mItems.length; i++) {
						View item = mInflater.inflate(
								R.layout.poi_sort_list_item,
								mPopWindowSortContentView, false);
						item.setTag(i);
						item.setOnClickListener(mOnClickListener);
						TextView tv = (TextView) item
								.findViewById(R.id.textViewPoiSortItemName);
						tv.setText(mItems[i].name);
						if (i == 0) {
							mPopWindowSortContentView.addView(item);
						} else {
							mPopWindowSortContentView
									.addView(createDivierView());
							mPopWindowSortContentView.addView(item);
						}
					}
				...
				mPopupWindowSort
						.showAtLocation(
								...
								loc[0]
										+ mTextViewSort.getWidth()
										- getContext()
												.getResources()
												.getDimensionPixelSize(
														R.dimen.poi_sort_list_item_width),
	...
	private void findViews() {
		...
		mPoiLoading = findViewById(R.id.progressBarPoiLoading);
	...
	private View createDivierView() {
		...
		v.setLayoutParams(new LinearLayout.LayoutParams(
				LinearLayout.LayoutParams.FILL_PARENT, mDividerHeight));
	...
	public static class SortItem {
		...
		public SortItem(String name, String value, boolean focus,
				String onSelected) {
			this.name = name;
			this.value = value;
			this.focus = focus;
