...
public class PickPhoneNumberView extends PickBaseView {
	...
	public PickPhoneNumberView(Context context) {
		...
		int left = (int) (res
				.getDimension(R.dimen.pick_phone_number_padding_left) + 0.5);
		int right = (int) (res
				.getDimension(R.dimen.pick_phone_number_padding_right) + 0.5);
		int top = (int) (res
				.getDimension(R.dimen.pick_phone_number_padding_top) + 0.5);
		int bottom = (int) (res
				.getDimension(R.dimen.pick_phone_number_padding_bottom) + 0.5);
	...
	public void initView(String contactName, long contactPhotoId,
		...
		View header = mLayoutInflater.inflate(R.layout.pickview_header_contact,
				this, false);
		TextView tvName = (TextView) header
				.findViewById(R.id.textViewContactName);
		String title = mContext.getString(
				R.string.call_out_title_name_muti_number, contactName,
				phoneNumberInfos.size());
		Logger.d(TAG, "!--->initView()-----title =" + title);
		tvName.setText(title);

		// ImageView imageViewAvatar = (ImageView)
		// header.findViewById(R.id.imageViewAvatar);
		// Drawable drawable = RomContact.loadContactDrawable(getContext(),
		// contactPhotoId);
		// if (drawable != null) {
		...
		for (int i = 0; i < phoneNumberInfos.size(); i++) {
			...
			View view = mLayoutInflater.inflate(
					R.layout.pickview_item_muti_numbers, mContainer, false);// 导入布局

			TextView tvPhoneNumber = (TextView) view
					.findViewById(R.id.textViewPhoneNumber_call);// 号码资源
			tvPhoneNumber.setText(phoneNumberInfo.getNumber());// 设置号码

			TextView noText = (TextView) view
					.findViewById(R.id.textViewNo_call);// 序号资源
			noText.setText((i + 1) + "");// 设置序号

			TextView tvPhoneNumberNote = (TextView) view
					.findViewById(R.id.textViewPhoneNumberNote_call);// 归属地资源
		...
		// add cancel button
		View buttom = mLayoutInflater.inflate(
				R.layout.pickview_buttom_muti_numbers, this, false);
		addBottomButton(buttom);// xd modify

		mRlCallOutCancel = (RelativeLayout) buttom
				.findViewById(R.id.rl_call_out_cancel);
		...
		mCountDownTimer = new CountDownTimer(countDownMillis, 100) {
			...
			public void onTick(long millisUntilFinished) {
				...
				int progress = (int) ((countDownMillis - millisUntilFinished)
						...
						.getMax());
				// Logger.d(TAG,
				// "!--->startCountDownTimer()--progress = "+progress);
				mProgressBarWaiting.setProgress(progress);

				mTvCallOutCancelTime.setText(((millisUntilFinished + 1) / 1000)
						+ "S");
			}
