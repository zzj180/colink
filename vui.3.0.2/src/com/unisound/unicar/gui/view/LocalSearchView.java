...
public class LocalSearchView extends PickBaseView {
	...
	public void initView(List<PoiInfo> infos, String category) {
		...
		dataformView = mLayoutInflater.inflate(
				R.layout.local_search_dataform_view, null, false);
		btn_more = mLayoutInflater.inflate(R.layout.list_item_load_more, null,
				false);
		LinearLayout layout = (LinearLayout) btn_more
				.findViewById(R.id.btn_more_local);
	...
	public void addMoreView() {
		...
		if (btn_more_use) {
			...
			View dataformView = mLayoutInflater.inflate(
					R.layout.local_search_dataform_view, null, false);
	...
	public View getLocalItemView(final PoiInfo infos) {
		...
		LinearLayout phone_layout = (LinearLayout) view
				.findViewById(R.id.phone_layout);
		LinearLayout distance_layout = (LinearLayout) view
		...
		TextView tv_distance = (TextView) view
				.findViewById(R.id.tv_localsearch_distance);
		...
		TextView has_online_reservation = (TextView) view
				.findViewById(R.id.has_online_reservation);// //是否在线订购
		...
		Logger.d(TAG, "!--->getLocalItemView--dist = " + dist
				+ "KM; getDistance = " + infos.getDistance());
		tv_distance.setText(mContext.getString(R.string.local_search_distance,
				dist));
		...
		phone_layout.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View arg0) {
				Logger.d(TAG, "phone_layout OnClick phone = "
						+ phone.getText().toString());
				if (mCallback != null) {
					String selectedItem = infos.getSeletItem();
					String callselectedItem = infos.getCallSelectItem();
					Logger.d(TAG, "--189--selectedItem = " + selectedItem
							+ ";callselectedItem = " + callselectedItem);
					mCallback.dissMissLocalSessionView(
							LocalSearchShowSession.CALL_BACK_CALL,
							callselectedItem);
				}
		...
		distance_layout.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View arg0) {
				Logger.d(TAG, "distance_layout OnClick distance"
						+ distance.getText().toString());
				if (mCallback != null) {
					String selectedItem = infos.getSeletItem();
					String callselectedItem = infos.getCallSelectItem();
					Logger.d(TAG, "--205--selectedItem = " + selectedItem
							+ ";callselectedItem = " + callselectedItem);

					mCallback
							.dissMissLocalSessionView(
									LocalSearchShowSession.CALL_BACK_NAVI,
									selectedItem);
				}

				// if (GaodeMap.hasGaodeMapClient(mContext)) {
				// GaodeUriApi.startNavi(mContext,
				// infos.getLocationInfo().getLatitude(),
				// infos.getLocationInfo().getLongitude(),
				// infos.getRegions()[0], 2, 0);
				// }else {
				// Toast.makeText(mContext, R.string.gaode_nofind_map,
