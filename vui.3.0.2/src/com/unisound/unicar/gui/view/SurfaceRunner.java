...
public abstract class SurfaceRunner extends SurfaceView implements
	 ...
	 *            The application context we're running in.
	 * @param attrs
	 *            Layout attributes for this SurfaceRunner.
	 */
	 ...
	 *            The application context we're running in.
	 * @param options
	 *            Options for this SurfaceRunner. A bitwise OR of SURFACE_XXX
	 *            constants.
	 */
	 ...
	 * 
	 * @param delay
	 *            The time in ms to sleep each time round the main animation
	 *            loop. If zero, we will not sleep, but will run continuously.
	 * 
	 *            <p>
	 *            If you want to do all your animation under direct app control
	 *            using {@link #postUpdate()}, just set a large delay. You may
	 *            want to consider using 1000 -- i.e. one second -- to make sure
	 ...
	 * desire.
	 * 
	 * Note that only one thread can ever draw into a Surface, so you should not
	 * draw into the Surface here if your normal rendering will be in another
	 * thread.
	 * 
	 * @param holder
	...
	/**
	 * This is called immediately before a surface is destroyed. After returning
	 * from this call, you should no longer try to access this surface. If you
	 * have a rendering thread that directly accesses the surface, you must
	 * ensure that thread is no longer touching the Surface before returning
	 * from this function.
	 * 
	 * @param holder
	...
	/**
	 * Pause the app. Applications must call this from their Activity.onPause()
	 * method.
	 */
	// public void onPause() {
	 ...
	 *            The flag to set.
	 * @param why
	 *            Short tag explaining why, for debugging.
	 */
	...
	/**
	 * Start the surface running. Applications must call this to set the surface
	 * going. They may use this to implement their own level of start/stop
	 * control, for example to implement a "pause" button.
	 */
	...
	/**
	 * Stop the surface running. Applications may call this to stop the surface
	 * running. They may use this to implement their own level of start/stop
	 * control, for example to implement a "pause" button.
	 */
	...
	/**
	 * Start the animation running. All the conditions we need to run are
	 * present (surface, size, resumed).
	 */
	...
	/**
	 * Stop the animation running. Our surface may have been destroyed, so stop
	 * all accesses to it. If the caller is not the ticker thread, this method
	 * will only return when the ticker thread has died.
	 */
	...
	/**
	 * Asynchronously schedule an update; i.e. a frame of animation. This can
	 * only be called if the SurfaceRunner was created with the option
	 * LOOPED_TICKER.
	 */
	 ...
	 *            doUpdate(), if there was a preceeding call to doUpdate().
	 */
	...
	/**
	 * Get a Bitmap which is the same size and format as the surface. This can
	 * be used to get an off-screen rendering buffer, for example.
	 * 
	...
	/**
	 * Get a Bitmap of a given size, in the same format as the surface. This can
	 * be used to get an off-screen rendering buffer, for example.
	 * 
	 * @param w
	...
	private static final int ENABLE_ALL = ENABLE_SURFACE | ENABLE_SIZE
			| ENABLE_STARTED;
	// Application handle.
