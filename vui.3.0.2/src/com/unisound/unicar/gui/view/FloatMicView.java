...
public class FloatMicView extends FloatView {
	...
	private OnTouchListener mOnTouchListener = new OnTouchListener() {
		...
		/**
		 * the location relative to the view, (the view top left corner is the
		 * origin)
		 */
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			case MotionEvent.ACTION_MOVE:
				if (Math.abs(mRawX - mLastPostionX) > mDef
						&& Math.abs(mRawY - mLastPostionY) > mDef) {
					mHasMoved = true;
					// update float view position param
					int newX = (int) (mRawX - mTouchStartX);
					int newY = (int) (mRawY - mTouchStartY);
					// Logger.d(TAG,
					// "!--->ACTION_MOVE--x = "+mRawX+"; y = "+mRawY+"; mTouchStartX = "+mTouchStartX+"; mTouchStartY = "+mTouchStartY);
					updateViewPosition(newX, newY);
				}
				break;
			case MotionEvent.ACTION_UP:
				Logger.d(TAG, "!--->ACTION_UP---x = " + mRawX + "; y = "
						+ mRawY + "; mTouchStartX = " + mTouchStartX
						+ "; mTouchStartY = " + mTouchStartY + "; mHasMoved = "
						+ mHasMoved);
	...
	private void requestAutoDock() {
		...
		Logger.d(TAG, "requestAutoDock---mWindowParams.x = " + x
				+ "; mScreenSize.x/2 = " + mScreenSize.x / 2);
	...
	private void saveViewPostion() {
		Logger.d(TAG, "!--->saveViewPostion---mWindowParams.x = "
				+ mWindowParams.x + "; mWindowParams.y = " + mWindowParams.y);
	...
	private void getViewPostion() {
		...
		int savedY = UserPerferenceUtil.getInputViewY(mContext,
				getFloatViewDefaultLocationY());
		Logger.d(TAG, "!--->getViewPostion------savedX = " + savedX
				+ "; savedY = " + savedY);
	...
	/**
	 * get FloatView default LocationY XD 20150901 added
	 * 
	 * @return
	 */
	private int getFloatViewDefaultLocationY() {
		int floatViewHight = mContext.getResources()
				.getDrawable(R.drawable.float_mic_normal).getIntrinsicHeight();
		int defaultY = (DeviceTool.getScreenHight(mContext) + mStatusBarHeight - floatViewHight) / 2; // mWindowSize.y
																										// /
																										// 2
																										// -
																										// getHeight()
		Logger.d(TAG, "!--->getFloatViewDefaultLoactionY--defaultY = "
				+ defaultY + "; floatViewHight = " + floatViewHight);
		return defaultY;
	}
