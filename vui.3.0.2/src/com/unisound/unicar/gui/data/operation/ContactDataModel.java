...
public class ContactDataModel {
	...
	private static final String ACTION_UPDATE_CONTACT = "com.colink.zzj.contact.done";
	...
	private static final String CONTACT_SELECT_ALL = Contacts.DISPLAY_NAME
	...
	private final static String[] PROJECTION_PHONE = new String[] {
			Phone.CONTACT_ID, Phone.NUMBER, Phone.TYPE, Phone.LABEL,
			Phone.IS_PRIMARY, Phone.IS_SUPER_PRIMARY };
	...
	private ConcurrentHashMap<Long, Integer> mHashContactIDIndex = new ConcurrentHashMap<Long, Integer>();
	private ConcurrentHashMap<Long, ArrayList<PhoneNumberInfo>> mPhones = new ConcurrentHashMap<Long, ArrayList<PhoneNumberInfo>>();
...
//		registerContentObserver(mPhoneUri);
		...
		filter.addAction(ACTION_UPDATE_CONTACT);
	...
	private BroadcastReceiver receiver = new BroadcastReceiver(){
		public void onReceive(Context context, android.content.Intent intent) {
			...
			if(ACTION_UPDATE_CONTACT.equals(action)){
				Logger.d(TAG, "update contact start");
				update();
			}
		};
	};
	...
	private void saveContactToFile(ArrayList<ContactInfo> l, ArrayList<PhoneNumberInfo> phones) {
		FileOutputStream fos = null;
		try {
			Logger.i(TAG,"--saveContactToFile--"
							+ mContactSavedFile.getPath().toString() + " "
			...
			for (int i = 0; i < l.size(); i++) {
				...
				JsonTool.putJSONObjectData(jObject, "hasPhoneNumber",String.valueOf(hasPhoneNumber));
			...
			for (int k = 0; phones != null && k < phones.size(); k++) {
				...
				JsonTool.putJSONObjectData(obj, "contactType",String.valueOf(phone.getContactType()));
				...
				JsonTool.putJSONObjectData(obj, "primaryValue",String.valueOf(phone.getPrimaryValue()));
				JsonTool.putJSONObjectData(obj, "superPrimaryValue",String.valueOf(phone.getSuperPrimaryValue()));
			...
			if (mHashContactIDIndex != null && mHashContactIDIndex.size() > 0) {
				...
				Iterator<Entry<Long, Integer>> i = mHashContactIDIndex
						.entrySet().iterator();
	...
	private class SyncRunnable implements Runnable {
		...
		public void run() {
			...
			sycSystemContact();
			Logger.d(TAG,"--start sync contact read mid:"+ (System.currentTimeMillis() - start));
			
			sycSystemPhoneNumber();
			Logger.d(TAG, "end sync contact read:"+ (System.currentTimeMillis() - start) + " ms");
	...
	private void sycSystemContact() {
		...
		Cursor contactCursor = query(mContext,
			...
			if (contactCursor.getCount() > 0) {
				...
				while (contactCursor.moveToNext()) {
					...
					String displayName = contactCursor
							.getString(CONTACT_DISPLAY_NAME_INDEX);
					int hasPhoneNumber = contactCursor
							.getInt(CONTACT_HAS_PHONE_NUMBER_INDEX);
					i.setDisplayName(displayName);
					ArrayList<String> extendedNameList = imeWordsExtender
							.extendImeNames(new String[] { displayName });
					...
					for (int j = 0; extendedNameList != null
							&& j < extendedNameList.size(); j++) {
					...
					if (true) {
						...
						mHashContactIDIndex.put(getContactId(i.getContactId(),i.getContactType()), index);
					} 
//					else {
...
//					}
		...
		if (phones != null) {
			if (phones.getCount() > 0) {
				while (phones.moveToNext()) {
					...
					numberInfo.setContactId(phones
							.getInt(PHONE_CONTACT_ID_INDEX));
					...
					numberInfo.setRawNumber(NumberFormat
							.getCleanPhoneNumber(numberInfo.getNumber()));
					numberInfo.setType(phones.getInt(PHONE_TYPE_INDEX));
					numberInfo.setPrimaryValue(phones
							.getInt(PHONE_IS_PRIMARY_INDEX));
					numberInfo.setSuperPrimaryValue(phones
							.getInt(PHONE_IS_SUPER_PRIMARY_INDEX));

					phoneList.add(numberInfo);
					
					Logger.i(TAG, "phone:" + numberInfo.toString());

					long contactid = getContactId(numberInfo.getContactId(),
							numberInfo.getContactType());
	...
	public  Cursor query(Context context, ContentResolver resolver,
