...
public class RomControl {
    ...
    public static void enterControl(Context context, String type, Object... params) {
        ...
        if (ROM_APP_LAUNCH.equals(type)) {
            ...
            RomSystemSetting.setBluetoothEnabled(context,true);
        } else if (ROM_CLOSE_BLUETOOTH.equals(type)) {
            RomSystemSetting.setBluetoothEnabled(context,false);
        	...
        	try {
        		RomSystemSetting.RaiseOrLowerVolume(context, false, Integer.parseInt(params[0].toString()));
			} catch (Exception e) {
			}
        } else if (ROM_INCREASE_VOLUMNE.equals(type)) {
        	try {
        		RomSystemSetting.RaiseOrLowerVolume(context, true, Integer.parseInt(params[0].toString()));
			} catch (Exception e) {
			}
        	...
        	try {
        		RomSystemSetting.setVolume(context, Integer.parseInt(params[0].toString()));
			} catch (Exception e) {
			}
