...
public class AMapPoiAroundSearchViewBake extends LinearLayout implements ISessionView {

    private static final String TAG = AMapPoiAroundSearchViewBake.class.getSimpleName();
    ...
    public AMapPoiAroundSearchViewBake(Context context) {
        this(context, null);
        mContext = context;
        LayoutInflater.from(mContext).inflate(R.layout.layout_poi_around_search_amap, this,true);
    ...
    private void getCurrentLocation() {
        ...
        if (mLocationInfo != null) {
            ...
            Logger.d(TAG, "getCurrentLocation--mFromAddress = " + mFromAddress + "; mFromLat = "
                    + mFromLat + "; mFromLng = " + mFromLng);

            mLatLonPoint =
                    new LatLonPoint(mLocationInfo.getLatitude(), mLocationInfo.getLongitude());
        } else {
            // 默认西单广场
    ...
    private void init() {
        if (aMap == null) {
            ...
            mOptions.anchor(0.5f, 1).icon(
                    BitmapDescriptorFactory.fromResource(R.drawable.ic_amap_point));
            mOptions.position(new LatLng(mLatLonPoint.getLatitude(), mLatLonPoint.getLongitude()));
    ...
    private void setSelectType() {
        ...
        ArrayAdapter<String> adapter =
                new ArrayAdapter<String>(mContext, android.R.layout.simple_spinner_item, itemTypes);
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        ...
        public void onMapClick(LatLng latng) {
            locationMarker =
                    aMap.addMarker(new MarkerOptions().anchor(0.5f, 1)
                            .icon(BitmapDescriptorFactory.fromResource(R.drawable.ic_amap_point))
    ...
    private OnPoiSearchListener mOnPoiSearchListener = new OnPoiSearchListener() {
        ...
        public void onPoiItemDetailSearched(PoiItemDetail result, int rCode) {
            ...
            if (rCode == 0) {
                if (result != null) {// 搜索poi的结果
                    if (detailMarker != null) {
                        ...
                        if (result.getDeepType() != null) {
                            ...
                            Logger.d(TAG, "onPoiItemDetailSearched--this poi has no deep info.");
                        }
                    }

                } else {
                    Logger.d(TAG, "onPoiItemDetailSearched--AMap no search result");
                ...
                ToastUtil.show(mContext, mContext.getString(R.string.amap_error_other) + rCode);
            }
        }
    ...
    private void showSuggestCity(List<SuggestionCity> cities) {
        ...
        for (int i = 0; i < cities.size(); i++) {
            infomation +=
                    "城市名称:" + cities.get(i).getCityName() + "城市区号:" + cities.get(i).getCityCode()
  ...
  /*  private void showProgressDialog() {
        if (progDialog == null) progDialog = new ProgressDialog(mContext);
...
*/
    ...
    protected void doSearchQuery() {
        ...
        if (mLatLonPoint != null) {
            Logger.d(TAG, "doSearchQuery---mLatLonPoint is not null, new PoiSearch");
            ...
            Logger.d(TAG, "doSearchQuery---mLatLonPoint is not null, searchPOIAsyn--begin");
            ...
            Logger.d(TAG, "doSearchQuery---mLatLonPoint is not null, searchPOIAsyn---end");
        }
