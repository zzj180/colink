...
public class GaodeUriApi {
    ...
    public static void showLocation(Context context, String title, String content, double lat,
        ...
        String data =
                "androidamap://viewMap?sourceApplication=ishuoshuo&poiname=" + content + "&lat="
                        + lat + "&lon=" + lng + "&dev=0";
        Logger.d(TAG, "URI " + data);
    ...
    public static void showRoute(Context context, String model, double slat, double slng,
            String sName, String sCity, double dLat, double dLng, String dName, String dCity) {
        ...
        int m = 2;
        String data =
                "androidamap://route?sourceApplication=vui_car_assistant&" + "slat=" + slat
                        + "&slon=" + slng + "&sname=" + sName + "&dlat=" + dLat + "&dlon=" + dLng
                        + "&dname=" + dName + "&" + "dev=0&m=2&t=" + m + "&showType=1";
     ...
     * @param toLng:目的地经度
     * @param name:POI名称
     * @param style:导航方式(0 速度快; 1 费用少; 2 路程短; 3 不走高速；4 躲避拥堵；5 不走高速且避免收费；6 不走高速且躲避拥堵；7 躲避收费和拥堵；8
     *        不走高速躲避收费和拥堵))
    ...
    public static void startNavi(Context context, double toLat, double toLng, String name,
            int style, int dev) {
        Logger.d(TAG, "startNavi:toLat " + toLat + ",toLng " + toLng + ",name " + name + ",style "
                + style + ",dev " + dev);
/*        AMapUri uri = new AMapUri("navi");
  ...
  //      String dat = uri.getDatString();
        String dat = "androidamap://navi?poiname="+name+"&lat="+toLat+"&lon="+toLng+"&dev=0&style=0";
        Logger.d(TAG, "dat=" + dat);
    ...
    public static void startNavi2(Context context, double toLat, double toLng, String name,
            int style, int dev) {
        Logger.d(TAG, "startNavi:toLat " + toLat + ",toLng " + toLng + ",name " + name + ",style "
                + style + ",dev " + dev);
        String URI = String.format("NAVI:%s,%s", toLng, toLat);
        Intent intentAmap = new Intent("com.autonavi.xmgd.action.NAVIGATOR");
        intentAmap.setData(Uri.parse(URI));
        intentAmap.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        try {
        	context.startActivity(intentAmap);
		} catch (Exception e) {
			e.printStackTrace();
		}
    ...
    public static void openAMap(Context context) {
       /* String data = "androidamap://route?sourceApplication=uniCarSolution";
        ...
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        context.startActivity(intent);*/
    	Intent intent=isexit(context,pkg);
    ...
    public static void goHomeCorp(Context context,String dest){
    	Intent mIntent = new Intent("android.intent.action.VIEW", android.net.Uri.parse("androidamap://navi2SpecialDest?sourceApplication=applicationName&backScheme=applicationScheme&dest="+dest));
