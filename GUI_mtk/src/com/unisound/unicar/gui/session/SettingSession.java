...
import com.unisound.unicar.gui.oem.RomApp;
...
public class SettingSession extends BaseSession {
	...
	private static final String SCREENOFF_PACKAGE = "com.zzj.coogo.screenoff";
	...
	public static final String MEIX_MAP = "com.mxnavi.mxnavi";

	private static final String STOP_FM = "action.colink.stopFM";

	public static final String AMAP_STOPNAVI = "com.amap.stopnavi";

	public static final String AMAP_COMMAND = "com.autonavi.minimap.carmode.command";
	private static final String AMAP_COMMAND_KEY = "NAVI";
	private static final String AMAP_EXIT = "APP_EXIT";
	/*
	 * private static final String AMAP_OPEN_TRAFFIC = "OPEN_TRAFFIC_BROADCAST";
	 * //路况播报打开 private static final String AMAP_CLOSE_TRAFFIC =
	 * "CLOSE_TRAFFIC_BROADCAST";//路况播报关闭 private static final String
	 * AMAP_ZOOM_IN = "MAP_ZOOM_IN"; //放大导航地图 private static final String
	 * AMAP_ZOOM_OUT = "MAP_ZOOM_OUT"; //缩小导航地图 private static final String
	 * AMAP_NORTH = "MAP_NORTH_MODEL"; //导航正北模式 private static final String
	 * AMAP_CAR = "MAP_CAR_MODEL"; //导航随车模式 private static final String AMAP_2D
	 * = "MAP_2D_MODEL"; //导航2D模式 private static final String AMAP_3D =
	 * "MAP_3D_MODEL"; //导航3D模式
	 */
	...
	/** isNeedDoOnTTSEnd, zzj 20151103 adde */
	...
	public void putProtocol(JSONObject jsonProtocol) {
		...
		if (SessionPreference.VALUE_SETTING_OBJ_QUESTION.equals(operands)) {
			if (SessionPreference.VALUE_SETTING_ACT_PLAY.equals(operator)) {
				...
				RomCustomerProcessing.playtts((WindowService) mContext,
						mAnswer, RomCustomerProcessing.TTS_END_RECOGNIER_START);
			} else if (SessionPreference.VALUE_SETTING_ACT_OPEN
					.equals(operator)) {
				...
				RomCustomerProcessing.playtts((WindowService) mContext,
						mAnswer, RomCustomerProcessing.TTS_END_RECOGNIER_START);
			} else if (SessionPreference.VALUE_SETTING_ACT_TOTAL
					.equals(operator)) {
				...
				mingPianView.setResouse(
						mContext.getString(R.string.question_total),
						R.anim.mingpian);
				addAnswerView(mingPianView);
				mAnswer = "     野狼战队有成员17名，分别是" + array[r % len] + ","
						+ array[(r + 1) % len] + "," + array[(r + 2) % len]
						+ "等等";
				RomCustomerProcessing.playtts((WindowService) mContext,
						mAnswer, RomCustomerProcessing.TTS_END_RECOGNIER_START);
			} else if (SessionPreference.VALUE_SETTING_ACT_SENLIN
					.equals(operator)) {
				...
				RomCustomerProcessing.playtts((WindowService) mContext,
						mAnswer, RomCustomerProcessing.TTS_END_RECOGNIER_START);
			} else if (SessionPreference.VALUE_SETTING_ACT_ZHAN
					.equals(operator)) {
				...
				RomCustomerProcessing.playtts((WindowService) mContext,
						mAnswer, RomCustomerProcessing.TTS_END_RECOGNIER_START);
			} else if (SessionPreference.VALUE_SETTING_ACT_CANG
					.equals(operator)) {
				...
				RomCustomerProcessing.playtts((WindowService) mContext,
						mAnswer, RomCustomerProcessing.TTS_END_RECOGNIER_START);
			} else if (SessionPreference.VALUE_SETTING_ACT_FENGHUO
					.equals(operator)) {
				...
				RomCustomerProcessing.playtts((WindowService) mContext,
						mAnswer, RomCustomerProcessing.TTS_END_RECOGNIER_START);
			} else if (SessionPreference.VALUE_SETTING_ACT_SHUAI
					.equals(operator)) {
				...
				RomCustomerProcessing.playtts((WindowService) mContext,
						mAnswer, RomCustomerProcessing.TTS_END_RECOGNIER_START);
			} else if (SessionPreference.VALUE_SETTING_ACT_QINHAI
					.equals(operator)) {
				...
				RomCustomerProcessing.playtts((WindowService) mContext,
						mAnswer, RomCustomerProcessing.TTS_END_RECOGNIER_START);
			} else if (SessionPreference.VALUE_SETTING_ACT_XIAO
					.equals(operator)) {
		...
		}// modif by zzj 20151207
		else if (SessionPreference.VALUE_SETTING_OBJ_CALL.equals(operands)) { // 电话
			Logger.d(TAG, "!--->VALUE_SETTING_OBJ_CALL---operator = "
					+ operator);
		...
		/* XD 20150717 delete End > */
			...
			if (SessionPreference.VALUE_SETTING_ACT_CLOSE.equals(operator)) {
				...
				Settings.System.putInt(mContext.getContentResolver(), "fm_switch", 1);
				mAnswer = mContext.getString(R.string.close_fm);
			}else if (SessionPreference.VALUE_SETTING_ACT_OPEN.equals(operator)) {
				Settings.System.putInt(mContext.getContentResolver(), "fm_switch", 0);
				mAnswer = mContext.getString(R.string.open_fm);
			}
		} else if (SessionPreference.VALUE_SETTING_OBJ_CARCORDER
				.equals(operands)) { // 行车记录仪
			if (SessionPreference.VALUE_SETTING_ACT_OPEN.equals(operator)) {
				mAnswer = mContext.getString(R.string.open_carcoder);
				RomControl.enterControl(mContext, RomControl.ROM_APP_LAUNCH,
								COM_ANDROID_CAMERA2,
								COM_ANDROID_CAMERA_CAMERA_ACTIVITY);
			} else if (SessionPreference.VALUE_SETTING_ACT_CLOSE
					.equals(operator)) {
				...
				// 应用程序位于堆栈的顶层
				if (tasksInfo.size() > 0) {
					if (COM_ANDROID_CAMERA_CAMERA_LAUNCHER.equals(tasksInfo
		...
		} else if (SessionPreference.VALUE_SETTING_OBJ_HOME.equals(operands)
				|| SessionPreference.VALUE_SETTING_OBJ_HOMEPAGE
						.equals(operands)) {// 返回主界面
			if (SessionPreference.VALUE_SETTING_ACT_OPEN.equals(operator)
					|| SessionPreference.VALUE_SETTING_ACT_SET.equals(operator)) {
		...
		} else if (SessionPreference.VALUE_SETTING_OBJ_CALL_LOG
				.equals(operands)) { // 打开通话记录
			if (SessionPreference.VALUE_SETTING_ACT_OPEN.equals(operator)) {
				mAnswer = mContext.getString(R.string.open_calllog);
				RomSystemSetting.openCallLog(mContext);
			}
		}else if (SessionPreference.VALUE_SETTING_OBJ_OBD.equals(operands)) {   //开启车载诊断系统
			
			if (SessionPreference.VALUE_SETTING_ACT_OPEN.equals(operator)){
				
				mContext.sendBroadcast(new Intent("com.example.enslaver.STARTOBD"));
				
			}else{
				
				mContext.sendBroadcast(new Intent("com.example.enslaver.CLOSEOBD"));  //关闭车载诊断系统
			}
			
		}else if (SessionPreference.VALUE_SETTING_OBJ_DEMANDOIL.equals(operands)) {
			
			mContext.sendBroadcast(new Intent("com.example.enslaver.DEMANDOIL"));	//查询油量
			
		}else if (SessionPreference.VALUE_SETTING_OBJ_CRASPEED.equals(operands)) {
			
			mContext.sendBroadcast(new Intent("com.example.enslaver.CRASPEED"));	//查询车速
			
		}else if (SessionPreference.VALUE_SETTING_OBJ_OBDSERVICE.equals(operands)) {
			
			if (SessionPreference.VALUE_SETTING_ACT_OPEN.equals(operator)){
				
				mContext.sendBroadcast(new Intent("com.example.enslaver.ConnBlueTooth"));	//连接蓝牙到车载诊断系统
				
			}else{
				
				mContext.sendBroadcast(new Intent("com.example.enslaver.CloseService"));  //退出车载诊断系统服务
				
			}	
			
		}else if (SessionPreference.VALUE_SETTING_OBJ_TRUNK.equals(operands)) {
			
			mContext.sendBroadcast(new Intent("com.example.enslaver.OpenTrunk"));	//开启后车箱
			
		}else if (SessionPreference.VALUE_SETTING_OBJ_NAVI.equals(operands)) { // 导航
			if (SessionPreference.VALUE_SETTING_ACT_GO_BACK_HOME
					.equals(operator)) {
				...
				case 1:
					...
					GaodeUriApi.goHomeCorp(mContext, "home");
					break;
				case 2:
				...
				case 5:
					Bundle b = new Bundle();
					b.putInt("CMD_TYPE_ENUM", 57);
					Intent i = new Intent("VC_TO_NAVI_COMMUNICATE_MESSAGE");
					String[] param = new String[3];
					param[0] = "1.0";
					param[1] = "";
					param[2] = "0";
					b.putStringArray("COMMUNICATE_INFO_CONTENT", param);
					i.putExtras(b);
					i.setFlags(32);
				...
				case 1:
					...
					GaodeUriApi.goHomeCorp(mContext, "corp");
					break;
				case 2:
				...
				mAnswer = mContext.getString(R.string.open_navi);
				int mapIndex = UserPerferenceUtil.getMapChoose(mContext);
				switch (mapIndex) {
				case 1:
					...
					break;
				case 3:
					RomApp.lanchApp(mContext, KLD_MAP);
					break;
				case 4:
					RomApp.lanchApp(mContext, GUIConfig.PACKAGE_NAME_GOOGLE_MAP);
					break;
				case 5:
					RomApp.lanchApp(mContext, MEIX_MAP);
					break;
				default:
				...
				mAnswer = mContext.getString(R.string.close_navi);
				try {
					mContext.sendBroadcast(new Intent(BAIDU_NAVI_QUITNAVI));
					mContext.sendBroadcast(new Intent(AMAP_STOPNAVI));
					Intent mIntent = new Intent(AMAP_COMMAND);
					mIntent.putExtra(AMAP_COMMAND_KEY, AMAP_EXIT);
					...
					mSessionManagerHandler.postDelayed(new Runnable() {
						...
						public void run() {
							...
							try {
								forceStopPackage(MEIX_MAP, context);
							} catch (Exception e) {
								e.printStackTrace();
							}
							
							try {
							  forceStopPackage(GUIConfig.PACKAGE_NAME_GOOGLE_MAP, context);
							} catch (Exception e) {
								e.printStackTrace();
							}
			...
			} else if (SessionPreference.VALUE_SETTING_ACT_OPEN_PLAY
					.equals(operator)) {
				int mapIndex = UserPerferenceUtil.getMapChoose(mContext);
				switch (mapIndex) {
				case 1:
					mContext.sendStickyBroadcast(new Intent(
							"com.amap.openroadradio"));
					/*
					 * Intent mIntent = new Intent(AMAP_COMMAND);
					 * mIntent.putExtra(AMAP_COMMAND_KEY, AMAP_OPEN_TRAFFIC);
					 * mContext.sendBroadcast(mIntent);
					 */
			...
			} else if (SessionPreference.VALUE_SETTING_ACT_CLOSE_PLAY
					.equals(operator)) {
				int mapIndex = UserPerferenceUtil.getMapChoose(mContext);
				switch (mapIndex) {
				case 1:
					mContext.sendStickyBroadcast(new Intent(
							"com.amap.closeroadradio"));
					/*
					 * Intent mIntent = new Intent(AMAP_COMMAND);
					 * mIntent.putExtra(AMAP_COMMAND_KEY, AMAP_CLOSE_TRAFFIC);
					 * mContext.sendBroadcast(mIntent);
					 */
					break;
				default:
			...
			} else if (SessionPreference.VALUE_SETTING_ACT_ZOOMIN
					.equals(operator)) { // 放大地图
				int mapIndex = UserPerferenceUtil.getMapChoose(mContext);
				switch (mapIndex) {
				case 1:
					mContext.sendStickyBroadcast(new Intent(
					...
					/*
					 * Intent mIntent = new Intent(AMAP_COMMAND);
					 * mIntent.putExtra(AMAP_COMMAND_KEY, AMAP_ZOOM_IN);
					 * mContext.sendBroadcast(mIntent);
					 */
					break;
				case 5:
					Bundle b = new Bundle();
					b.putInt("CMD_TYPE_ENUM", 6);
					Intent i = new Intent("VC_TO_NAVI_COMMUNICATE_MESSAGE");
					String[] param = new String[3];
					param[0] = "1.0";
					param[1] = "";
			...
			} else if (SessionPreference.VALUE_SETTING_ACT_ZOOMOUT
					.equals(operator)) { // 缩小地图
				int mapIndex = UserPerferenceUtil.getMapChoose(mContext);
				switch (mapIndex) {
				case 1:
					mContext.sendStickyBroadcast(new Intent(
					...
					/*
					 * Intent mIntent = new Intent(AMAP_COMMAND);
					 * mIntent.putExtra(AMAP_COMMAND_KEY, AMAP_ZOOM_OUT);
					 * mContext.sendBroadcast(mIntent);
					 */
					break;
				case 5:
					Bundle b = new Bundle();
					b.putInt("CMD_TYPE_ENUM", 5);
					Intent i = new Intent("VC_TO_NAVI_COMMUNICATE_MESSAGE");
					String[] param = new String[3];
					param[0] = "1.0";
					param[1] = "";
			...
			} else if (SessionPreference.VALUE_SETTING_ACT_CARUP
					.equals(operator)) {
				int mapIndex = UserPerferenceUtil.getMapChoose(mContext);
				switch (mapIndex) {
				case 1:
					...
					/*
					 * Intent mIntent = new Intent(AMAP_COMMAND);
					 * mIntent.putExtra(AMAP_COMMAND_KEY, AMAP_CAR);
					 * mContext.sendBroadcast(mIntent);
					 */
					break;
				default:
			...
			} else if (SessionPreference.VALUE_SETTING_ACT_NORTHUP
					.equals(operator)) {
				int mapIndex = UserPerferenceUtil.getMapChoose(mContext);
				switch (mapIndex) {
				case 1:
					...
					/*
					 * Intent mIntent = new Intent(AMAP_COMMAND);
					 * mIntent.putExtra(AMAP_COMMAND_KEY, AMAP_NORTH);
					 * mContext.sendBroadcast(mIntent);
					 */
					break;
				default:
				...
				case 1:
					...
					/*
					 * Intent mIntent = new Intent(AMAP_COMMAND);
					 * mIntent.putExtra(AMAP_COMMAND_KEY, AMAP_2D);
					 * mContext.sendBroadcast(mIntent);
					 */
					break;
				default:
				...
				case 1:
					...
					/*
					 * Intent mIntent = new Intent(AMAP_COMMAND);
					 * mIntent.putExtra(AMAP_COMMAND_KEY, AMAP_3D);
					 * mContext.sendBroadcast(mIntent);
					 */
					break;
				default:
			...
			RomCustomerProcessing.playtts((WindowService) mContext, mAnswer,
		...
		} else if (SessionPreference.VALUE_SETTING_OBJ_DISPLAY.equals(operands)) { // 关屏
			RomCustomerProcessing.playtts((WindowService) mContext, mAnswer,
					RomCustomerProcessing.TTS_END_WAKEUP_START);
			isNeedDoOnTTSEnd = true;
			...
			if (SessionPreference.VALUE_SETTING_ACT_OPEN.equals(operator)) {
				...
				mContext.sendBroadcast(intent);
				mAnswer = mContext.getString(R.string.open_radar);
				...
				mContext.sendBroadcast(intent);
				mAnswer = mContext.getString(R.string.close_radar);
			}
		 ...
		 * else if
		 * (SessionPreference.VALUE_SETTING_OBJ_AUTOLIGHT.equals(operands)) {
		 * //自动亮度开关 if
		 * (SessionPreference.VALUE_SETTING_ACT_OPEN.equals(operator)) { } else
		 * if (SessionPreference.VALUE_SETTING_ACT_CLOSE.equals(operator)) { }
		 * mAnswer = mContext.getString(R.string.open_display_settings);
		 * RomControl.enterControl(mContext,
		 * RomControl.ROM_OPEN_DISPLAY_SETTINGS); }
		 */else if (SessionPreference.VALUE_SETTING_OBJ_BLUETOOTH
				.equals(operands)) { // 蓝牙开关
			if (SessionPreference.VALUE_SETTING_ACT_OPEN.equals(operator)) {
				mAnswer = mContext.getString(R.string.open_bluetooth);
				RomControl
						.enterControl(mContext, RomControl.ROM_OPEN_BLUETOOTH);
			} else if (SessionPreference.VALUE_SETTING_ACT_CLOSE
					.equals(operator)) {
				mAnswer = mContext.getString(R.string.close_bluetooth);
				RomControl.enterControl(mContext,
						RomControl.ROM_CLOSE_BLUETOOTH);
			}
			...
			if (SessionPreference.VALUE_SETTING_ACT_SET.equals(operator)) {
				...
				RomControl.enterControl(mContext,
						RomControl.ROM_OPEN_TIME_SETTINGS);
			}
		} else if (SessionPreference.VALUE_SETTING_OBJ_LIGHT.equals(operands)) { // 亮度调节
			int light = 80;
			try {
				light = Settings.System.getInt(mContext.getContentResolver(),
			...
			} else if (SessionPreference.VALUE_SETTING_ACT_DECREASE
					.equals(operator)) {
				light -= 35;
				if (light > 255) {
					light = 255;
			...
			/*if (SessionPreference.VALUE_SETTING_ACT_INCREASE.equals(operator)) {
				...
				RomControl.enterControl(mContext,
						RomControl.ROM_INCREASE_VOLUMNE, 2);
			} else if (SessionPreference.VALUE_SETTING_ACT_DECREASE
					.equals(operator)) {
				mAnswer = mContext.getString(R.string.decrease_volumne);
				RomControl.enterControl(mContext,
						RomControl.ROM_DECREASE_VOLUMNE, 2);
			} else if (SessionPreference.VALUE_SETTING_ACT_MAX.equals(operator)) {
			...
			if (SessionPreference.VALUE_SETTING_ACT_OPEN.equals(operator)) {
				...
				RomControl.enterControl(mContext,
						RomControl.ROM_CLOSE_MODEL_VIBRA);
			}
		} else if (SessionPreference.VALUE_SETTING_OBJ_RINGTONE
				.equals(operands)) { // 声音设置
			if (SessionPreference.VALUE_SETTING_ACT_SET.equals(operator)) {
				mAnswer = mContext.getString(R.string.open_sound_setting);
				RomControl.enterControl(mContext,
						RomControl.ROM_OPEN_SOUND_SETTINGS);
			}
		} /*
		 * else if
		 * (SessionPreference.VALUE_SETTING_OBJ_ROTATION.equals(operands)) {
		 * //屏幕自动旋转开关 if
		 * (SessionPreference.VALUE_SETTING_ACT_OPEN.equals(operator)) { mAnswer
		 * = mContext.getString(R.string.open_rotation);
		 ...
		 */else if (SessionPreference.VALUE_SETTING_OBJ_WALLPAPER
				.equals(operands)) { // 壁纸设置
			if (SessionPreference.VALUE_SETTING_ACT_SET.equals(operator)) {
				mAnswer = mContext.getString(R.string.open_wallpaper_setting);
				RomControl.enterControl(mContext,
						RomControl.ROM_OPEN_WALLPAPER_SETTINGS);
			}
		} else if (SessionPreference.VALUE_SETTING_OBJ_WIFI.equals(operands)) { // WIFi开关
			if (SessionPreference.VALUE_SETTING_ACT_OPEN.equals(operator)) {
		...
		} else if (SessionPreference.VALUE_SETTING_OBJ_WIFI_SPOT
				.equals(operands)) { // WIFE热点开关
			if (SessionPreference.VALUE_SETTING_ACT_OPEN.equals(operator)) {
				mAnswer = mContext.getString(R.string.open_wifi_spot);
				RomControl
						.enterControl(mContext, RomControl.ROM_OPEN_WIFI_SPOT);
			} else if (SessionPreference.VALUE_SETTING_ACT_CLOSE
					.equals(operator)) {
				mAnswer = mContext.getString(R.string.close_wifi_spot);
				RomControl.enterControl(mContext,
						RomControl.ROM_CLOSE_WIFI_SPOT);
			}
			...
			RomCustomerProcessing.playtts((WindowService) mContext, mAnswer,RomCustomerProcessing.TTS_END_WAKEUP_START);
		}
		// 如果协议中包含OBJ_XXX,请在这里添加
		else {
			/*
			 * if
			 * (operator.equals(SessionPreference.VALUE_SETTING_ACT_OPEN_CHANNEL
			 * )) { } else
			 */if (SessionPreference.VALUE_SETTING_ACT_PREV.equals(operator)) { // 上一首
				isNeedDoOnTTSEnd = true;
				/*
				 * mAnswer = mContext.getString(R.string.previous); Intent
				 * intent = new Intent(CommandPreference.SERVICECMD);
				 * intent.putExtra(CommandPreference.CMDNAME,
				 * CommandPreference.CMDPREVIOUS);
				 * mMessageSender.sendMessage(intent, null);
				 */
			} else if (SessionPreference.VALUE_SETTING_ACT_NEXT
					.equals(operator)) { // 下一首
				isNeedDoOnTTSEnd = true;
				/*
				 * mAnswer = mContext.getString(R.string.next); Intent intent =
				 * new Intent(CommandPreference.SERVICECMD);
				 * intent.putExtra(CommandPreference.CMDNAME,
				 * CommandPreference.CMDNEXT);
				 * mMessageSender.sendMessage(intent, null);
				 */
			} else if (SessionPreference.VALUE_SETTING_ACT_PLAY.equals(operator)) { // 播放
				isNeedDoOnTTSEnd = true;
				/*
				 * mAnswer = mContext.getString(R.string.play); Intent intent =
				 * new Intent(CommandPreference.SERVICECMD);
				 * intent.putExtra(CommandPreference.CMDNAME,
				 * CommandPreference.CMDPLAY);
				 * mMessageSender.sendMessage(intent, null);
				 */
				RomCustomerProcessing.playtts((WindowService) mContext,
						mAnswer, RomCustomerProcessing.TTS_END_WAKEUP_START);
			} else if (SessionPreference.VALUE_SETTING_ACT_PAUSE
					.equals(operator)) { // 暂停
				isNeedDoOnTTSEnd = true;
				/*
				 * mAnswer = mContext.getString(R.string.pause); Intent intent =
				 * new Intent(CommandPreference.SERVICECMD);
				 * intent.putExtra(CommandPreference.CMDNAME,
				 * CommandPreference.CMDPAUSE);
				 * mMessageSender.sendMessage(intent, null);
				 */
				RomCustomerProcessing.playtts((WindowService) mContext,
						mAnswer, RomCustomerProcessing.TTS_END_WAKEUP_START);
			} else if (SessionPreference.VALUE_SETTING_ACT_RESUME
					.equals(operator)) { // 恢复播放
				isNeedDoOnTTSEnd = true;
				/*
				 * mAnswer = mContext.getString(R.string.recovery); Intent
				 * intent = new Intent(CommandPreference.SERVICECMD);
				 * intent.putExtra(CommandPreference.CMDNAME,
				 * CommandPreference.CMDPLAY);
				 * mMessageSender.sendMessage(intent, null);
				...
				RomCustomerProcessing.playtts((WindowService) mContext,
						mAnswer, RomCustomerProcessing.TTS_END_WAKEUP_START);
			} else if (SessionPreference.VALUE_SETTING_ACT_STOP
					.equals(operator)) { // 停止
				isNeedDoOnTTSEnd = true;
				/*
				 * mAnswer = mContext.getString(R.string.stop); Intent intent =
				 * new Intent(CommandPreference.SERVICECMD);
				 * intent.putExtra(CommandPreference.CMDNAME,
				...
				RomCustomerProcessing.playtts((WindowService) mContext,
						mAnswer, RomCustomerProcessing.TTS_END_WAKEUP_START);
			} else if (SessionPreference.VALUE_SETTING_ACT_SCREEN_DISPLAY
					.equals(operator)) {
				int light = 80;
				try {
				...
				Settings.System.putInt(mContext.getContentResolver(),
						Settings.System.SCREEN_BRIGHTNESS, light + 35);
			}
			// 云端的关闭既然不处理就删掉判断
			/*
			 * else if
			 * (SessionPreference.VALUE_SETTING_ACT_CLOSE.equals(operator)) {
			 * mAnswer = "已执行关闭"; Intent intent = new
			 * Intent(CommandPreference.SERVICECMD);
			 * intent.putExtra(CommandPreference.CMDNAME,
			 * CommandPreference.CMDSTOP); mMessageSender.sendMessage(intent,
			 * null); }
			 */
			else if (SessionPreference.VALUE_SETTING_OBJ_MUSIC_PREVIOUS_ITEM
					.equals(operands)) {
				if (SessionPreference.VALUE_SETTING_ACT_OPEN.equals(operator)) {
					Intent intent = new Intent(CommandPreference.SERVICECMD);
					intent.putExtra(CommandPreference.CMDNAME,
			...
			} else if (SessionPreference.VALUE_SETTING_ACT_PLAY_MESSAGE
					.equals(operator)) {
				Logger.d(TAG,
						"--VALUE_SETTING_ACT_PLAY_MESSAGE Execution Broadcast--");
			...
			} else if (SessionPreference.VALUE_SETTING_ACT_CANCEL
					.equals(operator)) {
				...
				view.setShowTitle(mContext.getString(R.string.no_support_cmd,
						mQuestion));
	...
	private View getStatusView(String text) {
		...
		View statusView = layoutInflater.inflate(
				R.layout.session_mic_status_view, null);
		TextView mTvSessionMicStatus = (TextView) statusView
				.findViewById(R.id.tv_session_mic_status);
	...
	public void onTTSEnd() {
		...
		mSessionManagerHandler
				.sendEmptyMessage(SessionPreference.MESSAGE_SESSION_DONE);
		if (isNeedDoOnTTSEnd) {
			doOnTTSEnd(); // zzj added 20151103
		}

		/* < zzj 20150717 delete begin */
	...
	private boolean isPlayerControl(String operator) {
	...
	private void doOnTTSEnd() {
		...
		if (SessionPreference.VALUE_SETTING_OBJ_SCREEN.equals(operands)
				|| SessionPreference.VALUE_SETTING_OBJ_DISPLAY.equals(operands)) { // 关屏
			if (SessionPreference.VALUE_SETTING_ACT_CLOSE.equals(operator)) {
				Intent intent = new Intent(Intent.ACTION_MAIN);
				intent.setClassName(SCREENOFF_PACKAGE, SCRRENOFF_ACTIVITY);
				intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				try {
					mContext.startActivity(intent);
				} catch (Exception e) {
			...
			PlayersControlManager.getInstance().sendPlayersControlCmd(mContext,
					mMessageSender, CommandPreference.CMDSTOP);
		} else if (SessionPreference.VALUE_SETTING_OBJ_COMPLAIN
				.equals(operands)) {
