...
public class RouteWaitingSession extends BaseSession {
	...
	public void putProtocol(JSONObject jsonProtocol) {
        ...
        if (WindowService.mLocationInfo != null) {
            ...
            if ("TW".endsWith(WindowService.language) && !TextUtils.isEmpty(mlLocationInfo.getAddress())){
            	mWaitingContentView.setStartPOI(ZHConverter.convert(mlLocationInfo.getAddress(), ZHConverter.TRADITIONAL));
			}else{
				mWaitingContentView.setStartPOI(mlLocationInfo.getAddress());
			}
            poiSearchModelProxy = POISearchModelProxy.getInstance(mContext);
            Logger.d(TAG, "Location info : " + mlLocationInfo + " toPoi : " + toPoi + " toCity " + toCity);
                ...
                public void onLocationChanged(LocationInfo info, ErrorUtil errorUtil) {
                    ...
                    // 定位成功
                        ...
                        if (mlLocationInfo != null) {
                            ...
                            if ("TW".endsWith(WindowService.language)){
                            	mWaitingContentView.setStartPOI(ZHConverter.convert(mlLocationInfo.getAddress(), ZHConverter.TRADITIONAL));
                			}else{
                				mWaitingContentView.setStartPOI(mlLocationInfo.getAddress());
                			}
    ...
    private IPoiListener mPoiListener = new IPoiListener() {
        ...
        public void onPoiSearchResult(List<PoiInfo> infos, ErrorUtil errorUtil) {
        	Logger.d(TAG, "--onPoiSearchResult : infos" + infos + "--ErrorUtil:"+errorUtil+"--:isSessionCanceled:"+isSessionCanceled+"--isRespknow:"+isRespknow);
