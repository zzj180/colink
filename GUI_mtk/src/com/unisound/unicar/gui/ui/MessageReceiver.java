    ...
    public void onReceive(Context context, Intent intent) {
		...
		//	Gps gcj02 = PositionUtil.gps84_To_Gcj02(Double.parseDouble(latlon[1]), Double.parseDouble(latlon[0])); //delete by zzj
			...
			case 1:
				...
				break;
			case 3:
				Gps gp = PositionUtil.gcj_To_Gps84(Double.parseDouble(latlon[1]),Double.parseDouble(latlon[0]));
				KLDUriApi.startNavi(context, gp.getWgLat(), gp.getWgLon(), poi);
				break;
			case 4:
			...
			case 5:
            	...
            	String dest_string = "(TND,2,0, "+latlon[1]+","+latlon[0]+","+poi+")";
			...
			if(readAccFile()){
				if(type.equals("bd")){
					...
					case 3:
						...
						KLDUriApi.startNavi(context, gcj.getWgLat(),gcj.getWgLon(), null);
						break;
					default:
						Intent navi = new Intent();
					...
					case 1:
						...
						break;
					case 3:
						KLDUriApi.startNavi(context, lat,lng, null);
						break;
					default:
			...
			if("StartMap".equals(cmd)){
				if(readAccFile()){
					int map = UserPerferenceUtil.getMapChoose(context);
					String poiName;
					String toLat;
					String toLon;
					switch (map) {
					case 1:
						...
						GaodeMap.showRoute(context, GaodeMap.ROUTE_MODE_DRIVING,Double.parseDouble(toLat),Double.parseDouble(toLon), "", poiName ,0);
						break;
					case 3:
						...
						KLDUriApi.startNavi(context, Double.parseDouble(toLat),Double.parseDouble(toLon), poiName);
						break;
					case 4:
						toLat = intent.getStringExtra("stand_latitude");
						toLon = intent.getStringExtra("stand_longitude");
			        	GoogleUriApi.showRoute(context, Double.parseDouble(toLat),Double.parseDouble(toLon));
			        	break;
					case 5:
						poiName = intent.getStringExtra("gaode_poiName");
						toLat = intent.getStringExtra("gaode_latitude");
					...
					default:
						...
						Intent navi = new Intent();
						navi.setData(Uri.parse(BDNAVI_PLAN_DEST + toLat+SPIT+toLon+SPIT+poiName));
						navi.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_SINGLE_TOP);
						try {
							context.startActivity(navi);
						} catch (Exception e) {
			...
			float lng=intent.getFloatExtra("lng", 0f);
			String address=intent.getStringExtra("address");
			Log.d("cldnavi", "lat="+lat+",lng="+lng);
			Gps gcj = PositionUtil.bd09_To_Gps84(lat, lng);
			KLDUriApi.startNavi(context, gcj.getWgLat(),gcj.getWgLon(), address);
		}
