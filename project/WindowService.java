...
public class WindowService extends Service {
	...
	public void onCreate() {
		...
		if (DeviceTool.getDeviceSDKVersion() > 13) {
			mViewRoot.setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN);// XD
																					// added
		} else {
		...
		mSessionManager = new GUISessionManager(this, mSessionContainer,
				mMicrophoneControl);// init framework & show prepare view
		...
		IntentFilter filter = new IntentFilter(
				GUIConfig.ACTION_SMS_SEND_SUCCESS);
		...
		getContentResolver()
				.registerContentObserver(
						Settings.System
								.getUriFor(SettingMapViewPagerActivity.MAP_INDEX),
						true, new ContentObserver(mUIHandler) {
							@Override
	...
	private void findViews() {
		...
		mMicrophoneDoresoControl = (MicrophoneControlDoresoView) mViewRoot
				.findViewById(R.id.microphoneControl_doreso); //XD added 20150828
	}
	...
	private void updateDisplaySize() {
		...
		Logger.d(TAG, "!--->updateDisplaySize:x " + mWindowSize.x + ",y "
				+ mWindowSize.y);
	}
	...
	public void dismiss() {
		...
		mUIHandler.postDelayed(mRecoverRunnable, 1500);
		/* xiaodong 20150805 added for Float Mic View End > */
	}
	...
	private Handler mUIHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case MSG_ON_RECORDING_RESULT:
				...
				mSessionManager.onTalkRecordingResult(result);
				break;
	...
	private void getGeneralGPS() {
					...
					if (locationInfo != null) {
						...
						if (mLocationInfo != null) {
							sendProtocolEvent(SessionPreference.EVENT_NAME_GENERAL_GPS,"{\"gpsInfo\":" + "\""
											+ mLocationInfo.getLatitude() + ","+ mLocationInfo.getLongitude()+ "\"}");
						}
					}
				}
			});
		...
		if (mLocationInfo != null) {
			sendProtocolEvent(SessionPreference.EVENT_NAME_GENERAL_GPS,
					"{\"gpsInfo\":" + "\"" + mLocationInfo.getLatitude() + "," + mLocationInfo.getLongitude() + "\"}");
		}
	}
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (Intent.ACTION_SCREEN_ON.equals(action)) {
				...
				if (!MessageReceiver.readAccFile()) {
					...
					context.sendBroadcast(new Intent("com.glsx.boot.ACCOFF"));
				}

			} else if (ACTION_ACC_ON.equals(action)) {
				accSwitch = true;
				TTSController.getInstance(mContext).init();
				if (UserPerferenceUtil.isWakeupEnable(mContext)) {
					sendProtocolEvent(
							SessionPreference.EVENT_NAME_SWITCH_WAKEUP,
							SessionPreference.EVENT_PROTOCAL_SWITCH_WAKEUP_START);
				...
				// 广联一键通
				context.sendBroadcast(new Intent("com.glsx.boot.ACCON"));
	...
	/*
	 * private void startFloatMicChecker(long delay) { //Logger.d(TAG,
	 * "startFloatMicChecker: delay " + delay); mHandler.postDelayed(mRunnable,
	 * delay); }
	 * 
	 * private void stopFloatMicChecker() { Logger.d(TAG,
	 * "stopFloatMicChecker"); setFloatMicEnable(false);
	 ...
	 * 
	 * @Override public void run() { if (mEnableFloatMic) {
	 * //PackageUtil.isHome(WindowService.this, mLauncherPackage) if
	 * (PackageUtil.isNeedHideFloatView(WindowService.this) ||
	 * isNeedHideMicFloatView) { hideFloatView(); } else { showFloatView(); }
	 * startFloatMicChecker(500); } else { hideFloatView(); } } };
	 */
	 ...
	 */
	/*
	 * public void setNeedHideMicFloatView(boolean isNeedHide){ Logger.d(TAG,
	 * "!--->setNeedHideMicFloatView---isNeedHide = "+isNeedHide);
	 * isNeedHideMicFloatView = isNeedHide; }
	 */
	/*
	 * public void showFloatView() { if (!mScreeFloatView.isShown()) {
	 * Logger.d(TAG, "!--->showFloatView---"); mScreeFloatView.show(); } }
	 * 
	 * public void hideFloatView() { if (mScreeFloatView.isShown()) {
	 * Logger.d(TAG, "!--->hideFloatView---"); mScreeFloatView.hide(); } }
	 */

	/*
	 * private void updateEnableFloatMic() { Logger.d(TAG,
	 * "updateEnableFloatMic---isShowFloatMicView = "
	 * +UserPerferenceUtil.isFloatMicEnable(this));
	 * setFloatMicEnable(UserPerferenceUtil.isFloatMicEnable(this)); }
	 */

	/*
	 * private void setFloatMicEnable(boolean enable) { Logger.d(TAG,
	 * "setFloatMicEnable: enable " + enable); mEnableFloatMic = enable; }
	 */
	...
	public void CreateDialog(Context context) {
		...
		((TextView) v.findViewById(R.id.textView1)).setText("导航到"
				+ mUserPreference.getString("address",
						UserPreference.PREPARE_NAVI_ADDRESS));
		builder.setView(v);
		Intent intent = new Intent("com.wanma.action.PLAY_TTS");
	...
	private void updateMapConfig() {
		int map_index = Settings.System.getInt(getContentResolver(),
				SettingMapViewPagerActivity.MAP_INDEX, 0);
		if (map_index == 0) {
