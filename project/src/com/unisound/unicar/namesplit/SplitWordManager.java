...
/*
 * 将非中文字符（全半角）和空格（全半角），视作分隔符
 * 连续的非中文字符视作1个
 *   a. 忽略空格，从字符串中提取所有长度大于等于2的中文字符串，作为中文名称内容，记为格式2
 *   b. 不忽略空格，从字符串中提取所有长度大于等于2的中文字符串，作为中文名称内容，也记为格式2
 * 对上面两步骤得到的内容进行去重
 * 
 * 丁森（地税） 处理后得到 丁森、地税，2个结果
 * 地税: 丁森 处理后得到 地税：丁森，2个结果
 */
