...
import com.unisound.unicar.gui.database.DatabaseHelper;
...
public class ChatDataBaseUtil {
	...
	private static final String[] cloums = new String[]{
		"_id", "text", "tts", "time"
	};
	...
	public static List<ChatObject> getExistChatDataFromDB(Context context){
		...
		try{
			Cursor cursor = context.getContentResolver().query(ChatHistoryTable.CONTENT_URI, 
					cloums, null, null, null);
			if(cursor != null) {
				cursor.moveToFirst();
				do {
