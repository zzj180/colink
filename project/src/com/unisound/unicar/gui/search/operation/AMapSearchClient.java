...
public class AMapSearchClient extends BaseMapSearchClient {
	...
	public void startSearch(PoiDataModel poiDataModel, IPoiListener poiListener) {
		...
		mCity = poiDataModel.getCity();
		Logger.d(TAG,
				"searchAMapPOIAsyn latitude : " + poiDataModel.getLatitude()
						+ ", lontitude : " + poiDataModel.getLontitude()
						+ ", city : " + mCity + ", poi : "
						+ poi);
		...
		Query query = new Query(poiDataModel.getPoi(),
				poiDataModel.getCategory(), poiDataModel.getCity());
	...
	private OnPoiSearchListener mPOISearchListener = new OnPoiSearchListener() {
		...
		public void onPoiSearched(PoiResult poiResult, int rCode) {
			...
			LocalSearchRouteConfirmShowSession.isGaoDe = true;
			Logger.d(TAG, "onPoiSearched:poiResult " + poiResult + ",rCode "
					+ rCode);
			onSearchResultReach();
			...
			if (rCode == 0) {
				if (poiResult == null || poiResult.getPois() == null
					...
					LocalSearchRouteConfirmShowSession.isGaoDe = false;
					PoiCitySearchOption option = new PoiCitySearchOption();
					option.city(mCity);
					option.keyword(poi);
					new BaiduSearchClient(mContext).searchInCity(option, mPoiListener);
					...
					errorUtil = DataModelErrorUtil
							.getErrorUtil(DataModelErrorUtil.RETURN_NORESULT);*/
				} else {
		...
		public void onPoiItemDetailSearched(PoiItemDetail result, int rCode) {
			...
			Logger.d(TAG, "onPoiItemDetailSearched:result " + result
					+ ",rCode " + rCode);
			onSearchResultReach();
		}
	};
	...
	private OnGeocodeSearchListener mGeocodeSearchListener = new OnGeocodeSearchListener() {
		...
		public void onRegeocodeSearched(RegeocodeResult result, int rCode) {
			...
			Logger.d(TAG, "onRegeocodeSearched:result " + result + ",rCpde "
					+ rCode);
			onSearchResultReach();
			...
			if (rCode == 0) {
				if (result == null || result.getRegeocodeAddress() == null) {
					...
					locationInfo = parse2LocationInfo(
							result.getRegeocodeAddress(), query == null ? null
									: query.getPoint());
				}
			} else {
				Logger.e(TAG, "Search poi failed! rCode:" + rCode);
				error = DataModelErrorUtil
		...
		public void onGeocodeSearched(GeocodeResult result, int rCode) {
			...
			Logger.d(TAG, "onGeocodeSearched: result " + result + ", rCpde "
					+ rCode);
			onSearchResultReach();
			ErrorUtil error = null;
			ArrayList<LocationInfo> locationInfos = null;
			if (rCode == 0) {
				if (result == null || result.getGeocodeAddressList() == null
					...
					locationInfos = new ArrayList<LocationInfo>();
					for (GeocodeAddress address : result
	...
	public void searchPOIAsyn(Query query, IPoiListener listener) {
		...
		requestSearch(new Runnable() {
			...
			public void run() {
				if (mPoiListener != null) {
					mPoiListener.onPoiSearchResult(null,DataModelErrorUtil.getErrorUtil(DataModelErrorUtil.SEARCH_POI_TIMEOUT));
				}
			}
		});
	...
	private static LocationInfo parse2LocationInfo(RegeocodeAddress address,
			LatLonPoint point) {
		if (address == null) {
			return null;
		}
	...
	public void getFromLocationAsyn(RegeocodeQuery query,
		...
		Logger.d(TAG, "getFromLocationAsyn: query " + query + ", listener "
				+ listener);
	...
	public void getFromLocationNameAsyn(GeocodeQuery query,
			ILocationListener listener) {
		Logger.d(TAG, "getFromLocationNameAsyn: query " + query + ", listener "
				+ listener);
		mLocationListener = listener;
		GeocodeSearch geocodeSearch = new GeocodeSearch(mContext);
		geocodeSearch.setOnGeocodeSearchListener(mGeocodeSearchListener);
	...
	public List<LocationInfo> getFromLocationName(GeocodeQuery query) {
		...
		ArrayList<LocationInfo> locationInfos = null;
		try {
			List<GeocodeAddress> addresses = geocodeSearch
	...
	public static LatLonPoint getOffsetGPS(double lat, double lng) {
		...
		try {
			...
			if (res.getStatusLine().getStatusCode() == 200) {
				...
				info = new LatLonPoint(Double.parseDouble(obj
						.getString(KEY_LAT)), Double.parseDouble(obj
						.getString(KEY_LNG)));
			}
