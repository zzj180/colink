...
public class BaiduSearchClient extends BaseMapSearchClient{
	...
	private OnGetPoiSearchResultListener mBaiduPoiSearchResultListener = new OnGetPoiSearchResultListener() {
		...
		public void onGetPoiResult(PoiResult result) {
			...
			} else if (result.error != null && ReverseGeoCodeResult.ERRORNO.NO_ERROR != result.error) {
	...
	public void startSearch(PoiDataModel poiDataModel, IPoiListener poiListener) {
		...
		Logger.d(TAG, "searchBaiduPOIAsyn latitude : " + poiDataModel.getLatitude() 
				+ ", lontitude : " + poiDataModel.getLontitude() + ", city : " + poiDataModel.getCity()
	...
	public boolean searchInCity(PoiCitySearchOption option, IPoiListener poiListener) {
		...
		requestSearch(new Runnable() {
			...
			public void run() {
				if (mPoiListener != null) {
					mPoiListener.onPoiSearchResult(
						null,
