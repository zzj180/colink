		...
		try {
			...
			if ("OK".equals(obj.getString(DianPing.STATUS))) {
				...
				for (int i = 0; i < businessArr.length(); i++) {
					...
					poiSearchResult.setBranchName(JsonTool.getJsonValue(poiObj, DianPing.BRANCH_NAME));
					poiSearchResult.setDistance(JsonTool.getJsonValue(poiObj, DianPing.DISTANCE, -1));
					...
					poiSearchResult.setRating(Float.parseFloat(JsonTool
						.getJsonValue(poiObj, DianPing.AVG_RATING, "0.0")));
					poiSearchResult.setHas_coupon(JsonTool.getJsonValue(poiObj, "has_coupon") == "0"? false : true);
					poiSearchResult.setHas_deal(JsonTool.getJsonValue(poiObj, "has_deal") == "0"? false : true);
					poiSearchResult.setHas_online_reservation(JsonTool.getJsonValue(poiObj, "has_online_reservation") == "0"? false : true);
					poiSearchResult.setAvg_price(Float.parseFloat(JsonTool.getJsonValue(poiObj, "avg_price", "0.0")));
	...
	protected void onPostExecute(List<PoiInfo> result) {
		...
		if (mListener != null && !isCancelled()) {
			if (result != null) {
				...
				mListener.onPoiSearchResult(result,DataModelErrorUtil.getErrorUtil(DataModelErrorUtil.RETURN_NORESULT));
			}
		}
	}
