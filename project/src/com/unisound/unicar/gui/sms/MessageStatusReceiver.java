...
public class MessageStatusReceiver extends BroadcastReceiver {
	...
	public static final String MESSAGE_STATUS_RECEIVED_ACTION = "com.unisound.intent.action.MESSAGE_STATUS_RECEIVED";
	...
	private void updateMessageStatus(Context context, Uri messageUri, byte[] pdu) {
		...
		try {
			if (cursor.moveToFirst()) {
				...
				Log.d(TAG, "updateMessageStatus: msgUrl=" + messageUri
							+ ", status=" + status);
				...
				Log.e(
					TAG,
					"[MessageStatusReceiver] Can't find message for status update: "
							+ messageUri);
			}
		} finally {
			cursor.close();
