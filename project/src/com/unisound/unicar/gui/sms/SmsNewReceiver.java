	...
	public void onReceive(Context context, Intent intent) {
		...
		if (intent.getAction().equals(ACTION)/*
			...
			Uri insert = context.getContentResolver().insert(
					Uri.parse("content://sms"), mValues);

			final String[] PORJECTION = new String[] { SMS._ID, SMS.TYPE,
					SMS.ADDRESS, SMS.BODY, SMS.DATE, SMS.THREAD_ID, SMS.READ,
					SMS.PROTOCOL, SMS.PERSON_ID };

			Cursor c = context.getContentResolver().query(insert, PORJECTION,
					null, null, null);
			if (c.moveToNext()) {
				if (mServiceStarted) {
