...
public class SmsSingleRecipientSender extends SmsMessageSender {
	...
	public void sendMessage(long token) throws SmsException {
		...
		if (messageCount == 0) {
			...
			throw new SmsException("SmsMessageSender.sendMessage: divideMessage returned " + "empty messages. Original message is \"" + mMessageText + "\"");
		}

		boolean moved = Sms.moveMessageToFolder(mContext, mUri, Sms.MESSAGE_TYPE_OUTBOX);
		if (!moved) {
			throw new SmsException("SmsMessageSender.sendMessage: couldn't move message " + "to outbox: " + mUri);
		}
		...
		for (int i = 0; i < messageCount; i++) {
			...
			Intent intent = new Intent(SmsSenderService.MESSAGE_SENT_ACTION, mUri, mContext, SmsSendReceiver.class);
		...
		try {
			...
			throw new SmsException("SmsMessageSender.sendMessage: caught " + ex + " from SmsManager.sendTextMessage()");
		}

		Log.d(TAG, "[SmsSingleRecipientSender]  sendMessage: address=" + mDest + ", threadId=" + mThreadId + ", uri=" + mUri + ", msgs.count=" + messageCount);
