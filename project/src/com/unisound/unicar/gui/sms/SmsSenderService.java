 ...
 * store
 * incoming messages to the database, update notifications, etc. without
 * blocking the
...
public class SmsSenderService extends Service {
	...
	public static final String ACTION_SERVICE_STATE_CHANGED = "com.unisound.intent.action.SERVICE_STATE";
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (mResultCode != 0) {
			Logger.v(TAG, "onStart: #" + startId + " mResultCode: " + mResultCode + " = " + translateResultCode(mResultCode));
		}
	...
	public synchronized boolean sendFirstQueuedMessage() {
		...
		if (c != null) {
			try {
				if (c.moveToFirst()) {
					...
					SmsMessageSender sender = new SmsSingleRecipientSender(this, address, msgText, threadId, status == Sms.STATUS_PENDING, msgUri);

					Logger.v(TAG, "sendFirstQueuedMessage " + msgUri + ", address: " + address + ", threadId: " + threadId);
					...
					try {
						...
						Logger.e(TAG, "!--->sendFirstQueuedMessage: failed to send message " + msgUri + ", catch :" + e.getMessage());
						mSending = false;
		...
		if (success) {
			...
			/*mToastHandler.post(new Runnable() {
				public void run() {
					Logger.d(TAG, "!--->sms_sent_success");
					sendBroadcast(new Intent(GUIConfig.ACTION_SMS_SEND_SUCCESS));
					Toast.makeText(SmsSenderService.this, getString(R.string.sms_sent_success), Toast.LENGTH_SHORT).show();
				}
			});*/
	...
	private void handleSmsSent(Intent intent) {
		...
		Logger.d(TAG, "handleSmsSent uri: " + uri + " sendNextMsg: " + sendNextMsg + " mResultCode: " + mResultCode + " = " + translateResultCode(mResultCode));
		...
		mToastHandler.post(new Runnable() {
			public void run() {
				if(isSuccess){
					...
					Toast.makeText(SmsSenderService.this, getString(R.string.smsMessageQueued), Toast.LENGTH_SHORT).show();
	...
	/**
	 * Move all messages that are in the outbox to the failed state and set them
	 * to unread.
	 ...
	 * <code>ContentResolver</code> and return the <code>Uri</code> of the
