...
public class ReceiveSMSView extends LinearLayout implements ISessionView{
	...
	public ReceiveSMSView(Context context, SmsItem mSmsItem, String scheduleType) {
		...
		Logger.d(TAG, "!--->ReceiveSMSView----mScheduleType="+mScheduleType+"; mName = "+mName+"; mNumber = "+mNumber+"; mMessage = "+mMessage);
	...
	private void findViews() {
		...
		View view = mLayoutInflater.inflate(R.layout.sms_received_view, this, true);
		LinearLayout ll = (LinearLayout) view.findViewById(R.id.ll_received_sms_view);
				
		mTvReceivedSMSContent = (TextView) ll.findViewById(R.id.tv_received_sms_content);
		...
		if (SessionPreference.VALUE_SCHEDULE_TYPE_INCOMING_SMS.equals(mScheduleType)){
			mBtnBroadcast.setVisibility(View.VISIBLE);
			if(GUIConfig.isShowSmsReplyButton){
				mBtnReply.setVisibility(View.GONE);
			}
		} else if (SessionPreference.VALUE_SCHEDULE_TYPE_INCOMING_SMS_OPERATOR.equals(mScheduleType)) {
		...
		if (mName != null && mName.length() >0) {
			setContent(mContext.getString(R.string.sms_received_title_from, mName));
		}else if (mNumber != null && mNumber.length() > 0) {
			setContent(mContext.getString(R.string.sms_received_title_from, mNumber));
		}
