...
public class PickPhoneNumberView extends PickBaseView {
	...
	public void initView(String contactName, long contactPhotoId, ArrayList<PhoneNumberInfo> phoneNumberInfos) {
		...
		String title = mContext.getString(R.string.call_out_title_name_muti_number, contactName, phoneNumberInfos.size());
...
//		}
		...
		for (int i = 0; i < phoneNumberInfos.size(); i++) {
			...
			View view = mLayoutInflater.inflate(R.layout.pickview_item_muti_numbers, mContainer, false);// 导入布局
		...
		mCountDownTimer = new CountDownTimer(countDownMillis, 100) {
			...
			public void onTick(long millisUntilFinished) {
				...
				int progress = (int) ((countDownMillis - millisUntilFinished) / (float) countDownMillis * mProgressBarWaiting
						.getMax());
				//Log.d(TAG, "!--->startCountDownTimer()--progress = "+progress);
