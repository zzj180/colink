...
//	Map<String, String> mSupportItemMap = new LinkedHashMap<String,String>();
	...
	public void findViews() {
		...
		mFunctionHelpAdapter = new FunctionHelpViewAdapter(mContext, getHelpTextInfoList(isShowRandomHelpText));
...
//		}
	 ...
	 * default： false; show resourceId arr[0] help text
	 * if true , must satisfy the needs of: resourceId arr[].size() > 1
	 */
	private ArrayList<FunctionHelpTextInfo> getHelpTextInfoList(boolean isShowRandomHelpText) {
		...
		Logger.d(TAG, "!--->getHelpTextInfoList()---isShowRandomHelpText--"+isShowRandomHelpText+"supprotList.size()  :"+mSupprotDomainList.size());
		if(mSupprotDomainList.size() > 0) {
			...
			for(int i = 0 ; i < numberArray.length ;i ++) {
				...
				if(isShowRandomHelpText){
					...
					Logger.d(TAG, "!--->getHelpTextInfoList()--i :"+i +" numberArray[i]:" +numberArray[i] +" s" +s);
				} else {
