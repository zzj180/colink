...
public class WeatherContentView extends FrameLayout implements ISessionView {
	...
	private void findViews() {
		...
		rl_weather_others = new RelativeLayout[] {rl_weather_other_first , rl_weather_other_second,
				rl_weather_other_third, rl_weather_other_fourth};
		tv_otherday_titles = new TextView[] {tv_otherday_title_first , tv_otherday_title_second,
				tv_otherday_title_third, tv_otherday_title_fourth};
		iv_otherday_weathers = new ImageView[] {iv_otherday_weather_first , iv_otherday_weather_second,
				iv_otherday_weather_third, iv_otherday_weather_fourth};
		tv_otherday_weathers = new TextView[] {tv_otherday_weather_first , tv_otherday_weather_second,
				tv_otherday_weather_third, tv_otherday_weather_fourth};
		tv_otherday_temperatures = new TextView[] {tv_otherday_temperature_first , tv_otherday_temperature_second,
	...
	public void setWeatherInfo(WeatherInfo weatherInfo, String mCity, String updateTime) {
		...
		imageViewSetImageResource(iv_weather_today_weather,getWeatherImage(todayWeather.getImageTitle()));
		...
		String carWashIndexDesc = getResources().getString(R.string.weather_car_wash) + carWashIndex;
	...
	private void otherDaySetUi(int index) {
		...
		if(tempDay.isFocusDay()){
			relativeLayoutSetBackgroudColor(rl_weather_others[index], getResources().getColor(R.color.weather_focus_day));
		}
		textViewSetText(tv_otherday_titles[index], getOtherDayTitle(mWeatherInfo,index));
		imageViewSetImageResource(iv_otherday_weathers[index], getWeatherImage(tempDay.getImageTitle()));
	...
	private String getOtherDayTitle(WeatherInfo weatherInfo, int index){
		...
		default:
			return sDayOfWeekNames[(mWeatherInfo.getWeatherDay(index).getDayOfWeek() + WEEK_TEMP_NUM) % WEEKDAY_NUM];
		}
	}
	
	/**
	 * get Weather Image resource id
	 * XD modify 20150729
	 * @param imgTitle
	 * @return
	 */
