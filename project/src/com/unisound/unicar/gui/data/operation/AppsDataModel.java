...
public class AppsDataModel {
	...
	public void sysApp() {//读取系统的app
		...
		if (apps != null && apps.size() > 0) {
			for (int i = 0; i < apps.size(); i++) {
				...
				AppInfo appInfo = new AppInfo(appLabel,(pinyins == null || pinyins.length == 0) ? "" : pinyins[0],
		...
		Log.d(TAG, "Get installed app:" + (t2 - t1));
		copyCache();
	...
	private void saveAppsToFile(ArrayList<AppInfo> mAppInfoList) {
		try {
			Log.i(TAG,"--saveMediasToFile--"
							+ mAppsSavedFile.getPath().toString() + " "
