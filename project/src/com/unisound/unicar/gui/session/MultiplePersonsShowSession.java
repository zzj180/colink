...
public class MultiplePersonsShowSession extends ContactSelectBaseSession {
	...
	public MultiplePersonsShowSession(Context context, Handler handle) {
		...
		Log.d(TAG, "!--->MultiplePersonsShowSession()-------");
	...
	public void putProtocol(JSONObject jsonProtocol) {
		...
		//JSONArray dataArray = getJsonArray(contactsJSON, SessionPreference.KEY_DATA_CONTACTS);//"person" --> "contacts"
		...
		if (dataArray != null) {
			for (int i = 0; i < dataArray.length(); i++) {
				...
				Logger.d(TAG, "!--->mDataItemProtocalList.add item = "+JsonTool.getJsonValue(item, SessionPreference.KEY_TO_SELECT));
				mDataItemProtocalList.add(JsonTool.getJsonValue(item, SessionPreference.KEY_TO_SELECT));
			}
			addAnswerViewText(mAnswer);

			Logger.d(TAG, "!--->mContactInfoList size = "+ mContactInfoList.size()+"; mPickPersonView = "+mPickPersonView);
