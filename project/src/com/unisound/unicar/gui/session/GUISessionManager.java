...
//	private AudioManager mAudioManager;
	...
	/** if waiting session is canceled, do not create GUI Sesion anymore 
	 * XD added 20150805 */
	...
	private Handler mSessionManagerHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case SessionPreference.MESSAGE_SESSION_DISMISS_WINDOW_SERVICE:
				Logger.d(TAG,"!--->mSessionManagerHandler----MESSAGE_SESSION_DISMISS_WINDOW_SERVICE");
			...
			case SessionPreference.MESSAGE_ADD_ANSWER_VIEW:
				...
				Log.d(TAG, "!--->----------MESSAGE_ADD_ANSWER_VIEW----msg.arg1 = "+msg.arg1+"-- fullScroll = "+fullScroll);
			...
			case SessionPreference.MESSAGE_UI_OPERATE_PROTOCAL:
				...
				Logger.d(TAG, "!--->MESSAGE_UI_OPERATE_PROTOCAL eventName="+eventName+"; protocal : "+ protocal);
			...
			case SessionPreference.MESSAGE_SEND_RESP_PROTOCAL:
				...
				String respState = bundleResp.getString(SessionPreference.PARAM_KEY_STATE_INCOMING);
				Logger.d(TAG, "!--->MESSAGE_SEND_RESP_PROTOCAL respParam : " + respParam+";respState = " +respState);
				if(mCurrentSession == null){
					respParam = null;
				}
				if(respState != null && respState.length() > 0 ){
...
/*		if (mAudioManager != null) {
			Logger.d(TAG, "!--->resetRingMode---STREAM_RING");
	//		mAudioManager.setStreamMute(AudioManager.STREAM_RING, false);
		}*/
	}
	
	public void onWakeUpSuccess() {
		Logger.d(TAG, "!--->onWakeUpSuccess");
		//mSessionManagerHandler.removeMessages(SessionPreference.MESSAGE_SESSION_DISMISS_WINDOW_SERVICE);  //XD added 2050819
	...
	public GUISessionManager(WindowService windowService,
...
//		mAudioManager = (AudioManager) windowService.getSystemService(Context.AUDIO_SERVICE);
		init();
		setListener();
	...
	private void init() {
		...
		mLoadingView = (LoadViewLinearLayout) View.inflate(mWindowService,
		...
		mLoadingView
				.setDispatchKeyEventListener(mLoadViewDispatchKeyEventListener);
		/* xd modify 20150706 End > */
		showMicStatusView(mWindowService.getString(R.string.mic_prepare));
	}

	/**
	 * mLoadView DispatchKeyEventListener 
	 * xd added 20150706
	 */
	private DispatchKeyEventListener mLoadViewDispatchKeyEventListener = new DispatchKeyEventListener() {

		@Override
		public boolean dispatchKeyEvent(KeyEvent event) {
			Logger.d(TAG, "!--->loadingView dispatchKeyEvent()---keyCode = " + event.getKeyCode());
			if (event.getKeyCode() == KeyEvent.KEYCODE_BACK) {
				long secondTime = System.currentTimeMillis();
				if (isOpenDoubleClick && (secondTime - firstTimeBackKeyClick > TIME_DOUBLE_CLICK_BACK)) {
					Logger.d(TAG, "!--->--SHIW TOAST--");
					Toast.makeText(mWindowService, R.string.toast_click_again_to_exit,Toast.LENGTH_SHORT).show();
	...
	public void showInitView(boolean isShow) {
		...
		showLoadingView(isShow, false,
				mWindowService.getString(R.string.grammar_data_compile),"");
	}

	/**
	 * show init failed View
	 * XD added 20150804
	 * @param isShow
	 * @param errorMsg
	 */
	public void showInitFailedView(String errorMsg){
		...
		showLoadingView(true, true,
				mWindowService.getString(R.string.error_init_title_failed), errorMsg);
	}
...
//	}
	...
	private void dismissWelcomeActivity() {
		Logger.d(TAG, "!--->dismissWelcomeActivity()----------");
		mWindowService.sendBroadcast(new Intent(
				WelcomeActivity.ACTION_FINISH_WELCOMEACTIVITY));
	}
	...
	private void dismissGUIMainActivity() {
		...
		mWindowService.sendBroadcast(new Intent(
				GUIMainActivity.ACTION_FINISH_GUIMAINACTIVITY));
	}
	...
	private void showMicStatusView(String status) {
		Logger.d(TAG, "!--->showMicStatusView()-----status = " + status+"; mCurrentSession = "+mCurrentSession);
		if(mCurrentSession != null && !(mCurrentSession instanceof IKnowButtonSession)){
			Logger.d(TAG, "!--->mCurrentSession is not null && is not IKnowButtonSession, no need show MicStatusView");
			return;
		}
		View view = null;
		mSessionViewContainer.removeAllSessionViews();
	...
	/**
	 * show Sms SentSuccess View
	 * xd added 20150709
	 * XD 20150818 modify for SMS status show & status to VUI 
	 */
	public void showSmsSendStatusView(int status){
		...
		case GUIConfig.SMS_STATUS_SEND_SUCCESS:
			...
			//onUiProtocal(SessionPreference.EVENT_NAME_SMS_SENT_STATUS, SessionPreference.EVENT_PROTOCAL_SMS_STATUS_SUCCESS);
		...
		case GUIConfig.SMS_STATUS_SEND_FAIL:
			...
			//send sms sent status to VUI play TTS
			//onUiProtocal(SessionPreference.EVENT_NAME_SMS_SENT_STATUS, SessionPreference.EVENT_PROTOCAL_SMS_STATUS_FAIL);
			onUiProtocal(SessionPreference.EVENT_NAME_PLAY_TTS,
					GuiProtocolUtil.getPlayTTSEventParam(msg,
							GuiProtocolUtil.VALUE_RECOGNIZER_TYPE_WAKEPU));
			break;
	...
	private PhoneStateListener mPhoneStateListener = new PhoneStateListener() {
		...
		public void onCallStateChanged(int state, String incomingNumber) {
			...
			case TelephonyManager.CALL_STATE_RINGING:
				...
				if(mCurrentSession != null){
					if(mCurrentSession instanceof RouteWaitingSession || mCurrentSession instanceof LocalsearchWaitingSession){
						Message msg = new Message();
				...
				mSessionManagerHandler.sendEmptyMessage(SessionPreference.MESSAGE_SESSION_DONE);//XD modify 20150805
			/*	Logger.d(TAG, "STREAM_MUSIC Volume before:"+ mAudioManager.getStreamVolume(AudioManager.STREAM_MUSIC));
				Logger.d(TAG, "STREAM_RING Volume before:"+ mAudioManager.getStreamVolume(AudioManager.STREAM_RING));
				mAudioManager.setStreamMute(AudioManager.STREAM_RING, true);
				Logger.d(TAG, "STREAM_MUSIC Volume after:"+ mAudioManager.getStreamVolume(AudioManager.STREAM_MUSIC));
				Logger.d(TAG, "STREAM_RING Volume after:"+ mAudioManager.getStreamVolume(AudioManager.STREAM_RING));*/
...
//					text = incomingNumber + mWindowService.getString(R.string.phone_answer_cancel);
...
//				Logger.d(TAG, "--personName-->" + personName);
				...
				String protocal = "{\"service\":\"cn.yunzhisheng.setting\",\"semantic\":{\"intent\":{\"confirm\":\"INCOMING_CALL\"}},\"code\":\"SETTING_EXEC\",\"number\":\""+mNumber+"\",\"rc\":0}";
				Logger.d(TAG, "!--->MESSAGE_UI_OPERATE_PROTOCAL eventName="+eventName+"; protocal : "+ protocal);
				mWindowService.sendProtocolEvent(eventName, protocal); //xd added 20150702
		...
		//		mAudioManager.setStreamMute(AudioManager.STREAM_RING, false);
			...
			case TelephonyManager.CALL_STATE_IDLE:
				Logger.d(TAG, "!--->--TelephonyManager.CALL_STATE_IDLE--mPhoneState = "+mPhoneState);
...
//					});
	...
	private ISMSReceiver mSMSReceiverListener = new ISMSReceiver() {
		...
		public void onMessageReveived(SmsItem msg) {
			...
			Logger.d(TAG, "!--->--Receive message--name = " +name+"; number"+number+"; content = "+tts);
	...
	private void sendReceivedSmsEvent(String number,String content){
		...
		//"{\"service\":\"cn.yunzhisheng.setting\",\"semantic\":{\"intent\":{\"confirm\":\"INCOMING_CALL\"}},\"code\":\"SETTING_EXEC\",\"type\":\"10086\"}"
		String protocal = "{\"service\":\"cn.yunzhisheng.setting\",\"semantic\":{\"intent\":{\"confirm\":\"INCOMING_SMS\"}},\"code\":\"SETTING_EXEC\",\"number\":\""+number+"\",\"content\":\""+content+"\",\"rc\":0}";
		Logger.d(TAG, "!--->MESSAGE_UI_OPERATE_PROTOCAL eventName="+eventName+"; protocal : "+ protocal);
		mWindowService.sendProtocolEvent(eventName, protocal); //xd added 20150702
	}
	...
	/**
	 * release One Shot Session When Click Session Mic Btn
	 * XD added 20150731
	 */
	...
	/**
	 * This method called on mic cancel button click or back key click.
	 * send cancel event: CTT
	 * cancel GUI session
	 * dismiss WindowService
	 * XD 20150813 modify do it on WindowService mUIHandler to keep message sequence
	 */
...
//		    mWindowService.dismiss();
	 ...
	 * cancel GUI session
	 * dismiss WindowService
	 * xiaodong added 20150701 & 
	 * 20150819 replaced by onGUISessionDone()
	 */
	/*public void dismissWindowService() {
		Logger.d(TAG, "!--->dismissWindowService-----");
	...
	private void onGUISessionDoneDelay(){
		...
		releaseCurrentSession();
		mSessionManagerHandler.sendEmptyMessageDelayed(SessionPreference.MESSAGE_SESSION_DISMISS_WINDOW_SERVICE, 
	...
	/**
	 * session canceled on Waiting Session
	 * XD 20150813 added
	 */
...
//	}
	...
	/**
	 * 语法数据编译完成 dimissView loadingView & show GUIMainActivity, if is first
	 * start, first start has finished.
	 */
	...
	/**
	 * is Need Show Mic Status Session View
	 * XD 20150724 added for Mic Recording view do not show sometimes
	 * @return
	 */
	private boolean isNeedShowMicStatusSessionView(){
		Logger.d(TAG, "!--->isNeedShowMicStatusSessionView()----mLastSessionDone = "+mLastSessionDone
				+"; mSessionStatus = "+mSessionStatus+"; mType = "+mType);
		if (mLastSessionDone
				|| TextUtils.isEmpty(mSessionStatus)
				|| SessionPreference.VALUE_SESSION_END.equals(mSessionStatus)
				|| (SessionPreference.VALUE_SESSION_SHOW.equals(mSessionStatus) 
						&& (SessionPreference.VALUE_SCHEDULE_TYPE_UNSUPPORT.equals(mType)
								|| SessionPreference.VALUE_TYPE_UNSUPPORT_END_SHOW.equals(mType))) 
			 ) {
	...
	public void onTalkRecordingPrepared() {
		...
		mSessionManagerHandler.removeMessages(SessionPreference.MESSAGE_SESSION_DISMISS_WINDOW_SERVICE);  //XD added 2050824
	...
	public void onTalkRecordingStart() {
		...
		if(isNeedShowMicStatusSessionView()){
			...
			if (SessionPreference.VALUE_SET_STATE_TYPE_WAKEUP_SUCCESS_DORESO == mWindowService.getWakeupSuccessType()) {
	...
	public void onTalkRecordingResult (String result, boolean isPartial) {
		if(!GUIConfig.isShowASRRecordResult){
			Logger.d(TAG, "!--->onTalkRecordingResult--GUIConfig isShowASRRecordResult false---result = "+result);
			return;
		}
	...
	public void onTTSPlayEnd(){
		Logger.d(TAG, "!--->onTTSPlayEnd---mCurrentSession = "+mCurrentSession);
		if(mCurrentSession != null){
			mCurrentSession.onTTSEnd();
		}
		
		/*< XD 20150817 added for SMS send status Begin */
		if(mCurrentSession == null && mSmsSendStatusView != null && mSmsSendStatus != GUIConfig.SMS_STATUS_SENDING){
		...
		if (mCurrentSession != null 
				&& mCurrentSession instanceof ReceiveSmsSession
	...
	private void releaseCurrentSession() {
		Logger.d(TAG, "releaseCurrentSession--mCurrentSession = "+mCurrentSession);
		setSessionEnd(false);//TODO: why setSessionEnd false? Is any problem?
		if (mCurrentSession != null) {
...
//		mAudioManager = null;
	...
	public boolean onBackPressed() {
		...
		if (mCurrentSession != null) {
			...
			View view = getMicStatusView(mWindowService
					.getString(R.string.mic_prepare));
	...
	private void createSession(String protocal) {
		...
		if (obj != null) {
			...
			JSONObject data = JsonTool.getJSONObject(obj,
					SessionPreference.KEY_DATA);
			mType = JsonTool.getJsonValue(data,
					SessionPreference.KEY_DATA_SCHEDULE_TYPE, "");
			...
			Logger.d(TAG, "!--->createSession---mType = " + mType+"; resultText = "+resultText + "; mSessionStatus = " + mSessionStatus);
		...
		//		mMicrophoneControl.onIdle(true); //added 20150703
				...
				Log.d(TAG, "!--->---VALUE_TYPE_SOME_PERSON------");
			...
			} else if (SessionPreference.VALUE_SCHEDULE_TYPE_ACTION_CANCEL.equals(mType)) {//xd added 
				Logger.d(TAG, "!--->VALUE_SCHEDULE_TYPE_ACTION_CANCEL-----");
				onGUISessionDone();
			 ...
			 */
				...
				if(SessionPreference.DOMAIN_ROUTE.equals(originType)){
					...
					isWaitingSessionCanceled = false;
					base = new LocalsearchWaitingSession(mWindowService, mSessionManagerHandler);//add tyz 0714
				}else {
				...
				base = new StockShowSession(mWindowService,
						mSessionManagerHandler);
			} else if (SessionPreference.VALUE_TYPE_MUTIPLE_LOCALSEARCH.equals(mType)) {//add tyz 0721
			...
			//TODO add by ch Traffic
				...
				Log.d(TAG, "tyz---------"+ "localsearch call ok");
				...
				base = new LocalSearchRouteConfirmShowSession(mWindowService,mSessionManagerHandler);
				Log.d(TAG, "tyz---------"+ "localsearch ok");
				...
				Log.d(TAG,
						"!--->--VALUE_TYPE_INPUT_CONTACT----TalkShowMiddleSession()---");
				...
				Logger.d(TAG, "!--->VALUE_MUTIPLE_LOCATION---isWaitingSessionCanceled = "+isWaitingSessionCanceled);
		...
		if (base != null) {
			...
			Logger.d(TAG,"!--->--mCurrentSession putProtocol---mCurrentSession = "+ mCurrentSession);
	...
	/**
	 * is need release last session(the confirm session) before new session to cancel timer.
	 * XD added 20150817
	...
	private boolean isNeedReleaseLastSession(BaseSession lastSession, BaseSession newSession) {
	    ...
	    if ((lastSession instanceof CallConfirmShowSession)	|| (lastSession instanceof MultipleNumbersShowSession) || ((lastSession instanceof SmsInputShowSession) && (newSession instanceof SmsShowSession))) {
			return true;
		} else {
			return false;
		}
	...
	/**
	 * show Asr Result Text On Mic View
	 * XD added 20150730
	 * @param result
	 */
	...
	/**
	 * show Asr Result Text On Session View
	 * XD added 20150730
	 * XD modify 20150906
	 * @param result
	 * @param isPartial
	...
	private void showAsrResultTextOnSessionView(String result, boolean isPartial){
		Logger.d(TAG, "!--->showAsrResultTextOnSessionView = "+result+"; isPartial = "+isPartial
				+"; mAsrResultSessionView = "+mASRResultSessionView);
		showAsrResultTextOnMicView("");
		if(mASRResultSessionView == null){
			...
			TextView mTvSessionMicStatus = (TextView) mASRResultSessionView.findViewById(R.id.tv_session_asr_result);
			...
			TextView mTvSessionMicStatus = (TextView) mASRResultSessionView
			    .findViewById(R.id.tv_session_asr_result);
			mTvSessionMicStatus.setText(result);
		}
	}
	...
	public View getMicStatusView(String status) {
		...
		if (SessionPreference.VALUE_SET_STATE_TYPE_WAKEUP_SUCCESS == mWindowService.getWakeupSuccessType()
			...
			LayoutInflater layoutInflater = (LayoutInflater) mWindowService
					.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
			mMicStatusView = layoutInflater.inflate(
					R.layout.session_mic_status_view, null);
			mTvSessionMicStatus = (TextView) mMicStatusView
