...
public class MultipleNumbersShowSession extends ContactSelectBaseSession {
	...
	public void putProtocol(JSONObject jsonProtocol) {
		...
		if (dataArray != null) {
			for (int i = 0; i < dataArray.length(); i++) {
				...
				Logger.d(TAG, "!--->putProtocol()--number = " + JsonTool.getJsonValue(item, SessionPreference.KEY_NUMBER));
				//phoneItem.setAttribution(getJsonValue(item, "numberAttribution"));
				mPhoneNumberInfoList.add(phoneItem);
				Logger.d(TAG, "!--->to_select = " + JsonTool.getJsonValue(item, SessionPreference.KEY_TO_SELECT));
	...
	protected IPickPhoneNumberViewListener mPickPhoneNumberListener = new IPickPhoneNumberViewListener() {
		...
		public void onOk() {
			...
			onUiProtocal(SessionPreference.EVENT_NAME_ON_CONFIRM_OK, mOkProtocal); //xd added 20150702
            mSessionManagerHandler.sendEmptyMessage(SessionPreference.MESSAGE_SESSION_DONE);
		}
		
		@Override
		public void onCancel() {
			Logger.d(TAG, "!--->mPickPhoneNumberListener---onCancel()----mOriginType = "+mOriginType);
		...
		public void onTimeUp() {
			Logger.d(TAG, "!--->mPickPhoneNumberListener---onTimeUp()-----mOriginType = "+mOriginType);
			if(SessionPreference.DOMAIN_CALL.equals(mOriginType)){
				executeCall(mSelectedPhoneNumberInfo.getNumber());
