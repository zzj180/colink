...
public class TrafficControlShowSession extends BaseSession {
	...
	public void putProtocol(JSONObject jsonProtocol) {
		...
		if(resultObject == null )
			return;
		mQuestion = JsonTool.getJsonValue(resultObject, "text", "") /*mContext.getString(R.string.to_stock)*/;
		addQuestionViewText(mQuestion);
		result = JsonTool.getJsonValue(resultObject, "header", "");
		JSONObject  results = JsonTool.getJSONObject(resultObject, "result");
		//无结果直接结束session
		if(results == null){
			mSessionManagerHandler.sendEmptyMessageDelayed(SessionPreference.MESSAGE_SESSION_DONE,500);
			return;
		}
		//限行规则
...
//		int t_week = mTrafficControlInfo.getWeek();
		 ...
		 * by chenhao
		 * 注：
		 * 这里直接与播报的TTs保持,周一返回的是7天的数据 周二只返回6天依次类推...
		 * 所有此处获取有问题,并且有可能今天去查询明天或者后天的信息，所有通过weekId
		 * 获取信息没法实现
		 */
	...
	public void onTTSEnd() {
		//XD 20150819 modify 
		mSessionManagerHandler.sendEmptyMessage(
				SessionPreference.MESSAGE_SESSION_DONE_DELAY);
	};
