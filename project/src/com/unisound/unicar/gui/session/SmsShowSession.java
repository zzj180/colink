...
public class SmsShowSession extends CommBaseSession {
	...
	public void putProtocol(JSONObject jsonObject) {
		...
		mSelectedPhoneNumberInfo.setNumber(JsonTool.getJsonValue(mDataObject, "number"));  //mJsonObject --> mDataObject
		String sendText = JsonTool.getJsonValue(mDataObject, "content");  //mJsonObject --> mDataObject
		if (sendText == null || "".equals(sendText)) {
			sendText = "";
		}
		Logger.d(TAG, "!--->-putProtocol()--number = "+mSelectedPhoneNumberInfo.getNumber()+"; sendText = "+sendText);
