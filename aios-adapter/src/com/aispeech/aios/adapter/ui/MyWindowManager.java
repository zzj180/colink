...
public class MyWindowManager {
	...
	private Handler mHandler = new Handler(AdapterApplication.getContext()
			.getMainLooper()) {
		@Override
		public void handleMessage(Message msg) {
			...
			case MSG_SET_BACKGROUD:
				smallWindow.setBackground(new BitmapDrawable(mContext
						.getResources(), mWindowBgBitmap));
				break;
			...
			case MSG_LOADING_WINDOW:
				...
				break;
			case MSG_RESUSE_VOLUME:
				if(!AdapterApplication.isMTK && AdapterApplication.accState && !isShowing){
	...
	private OnClickListener mOnClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.mic_view:
				...
				// UiEventDispatcher.notifyUpdateUI(UIType.Awake);
				UiEventDispatcher.notifyUpdateUI(UIType.Context, null,
						"您有什么需要？");
				break;
			case R.id.iv_close:
				UiEventDispatcher.notifyUpdateUI(UIType.Awake);
			...
			if (scrollState == AbsListView.OnScrollListener.SCROLL_STATE_IDLE) {
				if (view.getId() == R.id.resualtListView) {
					...
					if (lastIndex == view.getCount() - 1) {
						isListLastPage = !isViewCovered(view
								.getChildAt(lastIndex - firstIndex));
	...
	private OnMusicListItemClickListener mMusicListClickListener = new OnMusicListItemClickListener() {
		...
		public void onItemClick(List<Object> musicList, int position) {
			MusicNode.getInstance().getBusClient()
					.publish(AiosApi.Other.UI_CLICK);// 告知AIOS 语音交互停止
	...
	private OnNavigListItemClickListener mNavigListClickListener = new OnNavigListItemClickListener() {
		...
		public void onItemClick(List<Object> navigList, int position) {
			...
			HomeNode.getInstance().getBusClient()
					.publish(AiosApi.Other.UI_CLICK);// 语音交互停止

			try {
				MapOperateUtil.getInstance().startNavigation(
						(PoiBean) navigList.get(position));// 开始导航
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private void blur() {
		...
		mWindowBgBitmap = Bitmap.createBitmap(mWindowBgBitmap, 0, 0,
				mBitmapWidth, mBitmapHeith, matrix, true);

		RenderScript rs = RenderScript.create(mContext);

		// Bitmap bitmap = mWindowBgBitmap.copy(mWindowBgBitmap.getConfig(),
		// true);
		Allocation input = Allocation.createFromBitmap(rs, mWindowBgBitmap,
	...
	public void showNearbyUI(List<Object> pois, String title) {
		...
		smallWindow.showNavigationUI(pois, title, new NearbyAdapter(mContext,
				pois));
	}
	...
	public void showNavigationUI(List<Object> pois, String title) {
		...
		for (int i = 0; i < pois.size(); i++) {
			...
			map.put("nav_distance",
					((PoiBean) (pois.get(i))).getDisplayDistance());
			listItem.add(map);
		}
		SimpleAdapter mSimpleAdapter = new SimpleAdapter(mContext, listItem,
				R.layout.nav_item, new String[] { "nav_index", "nav_address",
						"nav_area", "nav_distance" }, new int[] {
						R.id.nav_index, R.id.nav_address, R.id.nav_area,
						R.id.nav_distance });
	...
	public void updateListNextPage() {
		...
		int index_add = isViewCovered(lastItem) ? (lastIndex - firstIndex)
		...
		if (mNavListCount > lastIndex + 1) {
			if ((firstIndex + index_add + index_add) > mNavListCount - 1) {// last
																			// page
				if (mNavListCount >= lastIndex + 1) {
	...
	public void updateListPrePage() {
		...
		int length;
		int size = (int) Math.ceil((double) mListView.getHeight()
				/ (double) firstView.getHeight());
		length = size;
	...
	public void showPhoneListUI(List<PhoneNode.PhoneItem> list, String title) {
		...
		SimpleAdapter mSimpleAdapter = new SimpleAdapter(mContext, listItem,
				R.layout.phone_item, new String[] { "phone_index",
						"phone_name", "phone_number", "phone_area" },
				new int[] { R.id.phone_index, R.id.phone_name, R.id.phone_num,
						R.id.phone_area });
	 ...
	 *            要拨打的联系人归属地
	 */
	public void updatePhoneWait(final String name, final String number,
			String address) {
		createSmallWindow();
		...
		smallWindow.setPhoneCancelButtonClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				Log.i(TAG, "点击取消按钮执行");
				stopPhoneWait(true);
				PhoneNode.getInstance().getBusClient()
						.publish(AiosApi.Other.UI_CLICK);// 停掉AIOS交互，
				removeSmallWindow();// 移除悬浮窗
			}
		}, "取消");
	}
	 ...
	 *            归属地
	 */
	public void isAcceptPhone(final String name, final String number,
			String address) {
		createSmallWindow();
		mContactNumber = number;
	...
	public void sendBroadCastToCallPhone(String phoneNum) {
		SendBroadCastUtil.getInstance().sendBroadCast(
				"com.colink.service.TelphoneService.TelephoneReceive",
				"number", phoneNum);
	...
	public void sendBroadCastToAcceptPhone(String phoneNum) {
		SendBroadCastUtil.getInstance().sendBroadCast(
				"com.colink.service.TelphoneService.TelephoneAnswerReceive",
				"number", phoneNum);
		PhoneNode.getInstance().getBusClient().publish(AiosApi.Other.UI_CLICK);
		removeSmallWindow();
	}
	...
	public void sendBroadCastToRejectPhone(String phoneNum) {
		SendBroadCastUtil.getInstance().sendBroadCast(
				"com.colink.service.TelphoneService.TelephoneHandupReceive",
				"number", phoneNum);
		PhoneNode.getInstance().getBusClient().publish(AiosApi.Other.UI_CLICK);
		removeSmallWindow();
	}
	 ...
	 *            UI上要显示的搜索限行的关键字
	 */
	public void showVehiclerestrictionUI(VehicleRestrictionBean vehBean,
			String title) {
