...
public class SystemOperateUtil {
    ...
    private int tmpVolume = 0;//系统当前的音量
    ...
    public boolean getHotPortState() {
...
//        currentBrightness = Settings.System.getInt(con.getContentResolver(), Settings.System.SCREEN_BRIGHTNESS) + 51;//获取当前屏幕亮度-20
    ...
    private String playSoundTips(final int type) {
        setMute(false);
        int currentVolume = PreferenceHelper.getInstance().getVolume() / 3; //系统当前音量/3
        ...
        switch (type) {
            case TIPS_MUSIC_RAISE://增大音量
                if (currentVolume >= 4) {
                    ...
                    mAudioManager.setStreamVolume(AudioManager.STREAM_NOTIFICATION, currentVolume * 3, 0);
