...
public final class DeviceUtil {
	...
	private final static String FAN_DEVICES = "/sys/devices/virtual/input/input1/fan_power";
	...
	public static synchronized String getSDcardID() {
		try {
			String sdCid = null;
			String[] memBlkArray = new String[] { "/sys/block/mmcblk0",
					"/sys/block/mmcblk1", "/sys/block/mmcblk2" };
			for (String memBlk : memBlkArray) {
				File file = new File(memBlk);
				if (file.exists() && file.isDirectory()) {
					Process cmd = Runtime.getRuntime().exec(
							"cat " + memBlk + "/device/cid");
	...
	/**
	 * get mac address try to get mac address from WifiManager firstly, if wifi
	 * is disabled, enable it firstly then try to get mac address in a loop with
	 * timeout 5s
	 * 
	 * @param context
	 *            上下文
	...
	public static void controlFan(String value) {
		try {
