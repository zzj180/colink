...
public class PhoneNode extends BaseNode {
	...
	private static final String ACTION_DIAL = "com.colink.service.TelphoneService.TelephoneReceive"; // add
																										// by
																										// zzj
	...
	public void registerBtReceiver() {
		if (null == btReceiver)
			btReceiver = new BtPhoneReceiver();
		...
		// pin wang
		/*
		 * mContext.getContentResolver().registerContentObserver(Uri.parse(
		 * "content://com.colink.bluetoothe/bluetootheonline"), false,new
		 * ContentObserver(new Handler()) {
		 * 
		 * private Cursor query; String columnName = "online";
		 * 
		 * @SuppressLint("NewApi")
		 * 
		 * @Override public void onChange(boolean selfChange, Uri uri) {
		 * super.onChange(selfChange, uri); query =
		 * mContext.getContentResolver().query(uri, null, null, null, null); if
		 * (query.moveToNext()) { int state =
		 * query.getInt(query.getColumnIndex(columnName)); switch (state) { case
		 * 0: isConnect = false; //收到蓝牙已断开消息，通知aios //changed by ydj on
		 * 2016.3.24 // mHandler.sendEmptyMessage(PHONE_BLUETOOTH_DISCONNECTED);
		 * publishSticky("phone.bluetooth.state", "disconnected"); //changed by
		 * ydj on 2016.3.24 break; case 1: isConnect = true; //收到蓝牙已连接消息，通知aios
		 * //changed by ydj on 2016.3.24 //
		 * mHandler.sendEmptyMessage(PHONE_BLUETOOTH_CONNECTED);
		 * publishSticky("phone.bluetooth.state", "connected"); //changed by ydj
		 * on 2016.3.24 break; default: break; } } } } );
		 */
		// pin wang
	}
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(AIOS_BTSTATE_CONNECTED)) {
				...
				case TelephonyManager.CALL_STATE_RINGING:
					 ...
					 * msg.what=PHONE_INCOMING_CALL; mHandler.sendMessage(msg);
					 */
					// deleted by ydj on 2016.4.23
					...
					list.add(number);
					list.add(addressIn);
					UiEventDispatcher.notifyUpdateUI(UIType.PhoneAnswerStatus,
							list, "");
					if (null != bc) {
						bc.publish("phone.incomingcall.state", "ringing",
								number, name);
					}
	...
	public void onMessage(String topic, byte[]... parts) throws Exception {
		...
		if (topic.equals("data.sync.state")
				&& "contacts".equals(new String(parts[0]))) {
	...
	@Override
	public BusClient.RPCResult onCall(final String url, final byte[]... args)
			throws Exception {
		...
		if (url.equals("/phone/outgoingcall")) {
			...
			if (UiEventDispatcher.isListViewFirstPage()) {
				return new BusClient.RPCResult(null,
			...
			// modify by zzj
			// SendBroadCastUtil.getInstance().sendBroadCast(AIOS_OUTGOING_DIAL,
			// "number", phoneNum);
			...
			if (enable == 1) {
				...
				tmpIntent.putExtra("name", phoneName);
				tmpIntent.putExtra("number", phoneNum);
	 ...
	 */
	private void showList(byte[]... args) throws JSONException,
		...
		phoneListTitle = "联系人";
		UiEventDispatcher
				.notifyUpdateUI(UIType.PhoneList, list, phoneListTitle);
	}
	 ...
	 * @throws UnsupportedEncodingException
	 */
	private void callPhone(byte[] arg) throws JSONException,
			UnsupportedEncodingException {
		...
		String address = LocationDBHelper.getInstance().findPhoneAreaByNumber(
				phoneNum);
		if (DOMAIN_FALG) {
			List<Object> list = new ArrayList<Object>();
			list.add(phoneName);
