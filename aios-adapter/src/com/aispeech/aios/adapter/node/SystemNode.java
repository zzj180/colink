...
public class SystemNode extends BaseNode {
	...
	public WakeLock getWakeLock() {
		if (wl == null) {
			PowerManager pm = (PowerManager) AdapterApplication.getContext()
					.getSystemService(Context.POWER_SERVICE);
			wl = pm.newWakeLock(PowerManager.FULL_WAKE_LOCK
					| PowerManager.ACQUIRE_CAUSES_WAKEUP, AdapterApplication
	 ...
	 */
	private String onCallCommand(String url, String APPName)
			throws UnsupportedEncodingException {
		...
		if (url.equals(AiosApi.System.OPEN_APP)) {// 打开第三方APP
			if (APPName.equals("行车记录仪")) {// 打开行车记录仪
				...
				UITimer.getInstance().executeAppTask(new UITimerTask(),
						UITimer.DELAY_MIDDLE);
				AIMusic.play();
			...
			try {
				return SystemOperateUtil.getInstance()
						.setScreenBrightnessDown();
			} catch (Settings.SettingNotFoundException e) {
				e.printStackTrace();
				return "调节亮度失败";
			}
			...
			if (mRpcRecall.getByteList().size() > 0) {// 取得指定的播放模式
				String mode = StringUtil.getEncodedString(mRpcRecall
						.getByteList().get(0));
